...
async function applyMigration() {
  ...
  try {
    ...
    for (const table of criticalTables) {
      try {
        const result = await prisma.$queryRawUnsafe<Array<{ tablename: string; rowsecurity: boolean }>>(`
        ...
        console.log(`   ⚠️  ${table}: Could not verify (${error instanceof Error ? error.message : String(error)})`);
      }
    }
