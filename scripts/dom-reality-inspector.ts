...
import type { DOMRealityReport, DOMIssue, DOMMetrics, InspectionConfig } from "./dom-reality-types";
...
export async function inspectDOMReality(config: InspectionConfig): Promise<void> {
  ...
  try {
    ...
    for (const route of config.routes) {
      ...
      for (const viewport of config.viewports) {
        ...
        for (const theme of themes) {
          const report = await inspectRoute(browser, route, config, viewport, theme);
    ...
    const summary = {
      ...
      criticalIssues: allReports.reduce(
        (sum, r) => sum + r.issues.filter((i) => i.severity === "critical").length,
        0
      ),
      warnings: allReports.reduce(
...
if (require.main === module) {
  const config: InspectionConfig = {
    routes: ["/", "/signup", "/pricing", "/docs", "/console", "/playground", "/trust"],
    viewports: [
