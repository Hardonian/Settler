      ...
      lines.forEach((line, index) => {
        if (line.match(/console\.(log|error|warn|debug)/)) {
          ...
          addResult({
            ...
            line: index + 1,
            recommendation:
              "Use proper logger (logInfo, logError, etc.) instead of console methods",
          });
        }
      });
    }
...
function checkDependencies() {
  ...
  try {
    const auditOutput = execSync("npm audit --json --audit-level=moderate 2>&1", {
      encoding: "utf-8",
    });
    ...
    if (audit.vulnerabilities && typeof audit.vulnerabilities === "object") {
      ...
      if (critical > 0 || high > 0) {
        addResult({
          ...
          recommendation:
            'Run "npm audit fix" to resolve vulnerabilities. Review changes before committing.',
        });
      }
    }
  } catch (error: any) {
