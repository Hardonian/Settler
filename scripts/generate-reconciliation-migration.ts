...
class ReconciliationMigrationGenerator {
  ...
  generate(): string {
    ...
    this.migrationSQL.push('-- It reconciles differences between expected and live database state.');
    this.migrationSQL.push('-- ============================================================================\n');
  ...
  private generateIndexSQL(issues: VerificationResult[]) {
    ...
    for (const issue of issues) {
      ...
      if (parts.length >= 3) {
        ...
        this.migrationSQL.push(`CREATE INDEX IF NOT EXISTS ${indexName} ON ${table} (${columnList});`);
      }
    }
...
async function main() {
  const resultsPath = process.argv[2] || 
    path.join(__dirname, '../supabase/backend-verification-results.json');
