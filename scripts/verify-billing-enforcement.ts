...
async function verifyBilling() {
  ...
  for (const file of routeFiles) {
    ...
    if (relativePath.includes('/webhook')) {
      const hasSignatureAuth = content.includes('stripe-signature') || 
    ...
    if (relativePath.includes('/cron/') || relativePath.includes('/internal/')) {
      const hasSecretAuth = content.includes('CRON_SECRET') ||
    ...
    const hasBilling = hasBillingGate || hasRequireSubscription || hasBillingEnforcement || hasPublic || hasFree;
