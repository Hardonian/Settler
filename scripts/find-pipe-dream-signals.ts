...
function findPipeDreamSignals(): PipeDreamSignal[] {
  ...
  const featureMatches = readmeContent.matchAll(
    /(?:supports?|provides?|includes?|features?|enables?)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi
  );
  ...
  if (fs.existsSync(envExample)) {
    ...
    for (const envVar of envVars) {
      ...
      const envRefs = codeContent.match(
        new RegExp(`process\\.env\\.${envVar}|\\$\\{.*${envVar}.*\\}`, "gi")
      );
