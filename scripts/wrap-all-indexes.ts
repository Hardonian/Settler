...
function wrapIndexStatements(filePath: string): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^\s*CREATE\s+(UNIQUE\s+)?INDEX\s+IF\s+NOT\s+EXISTS\s+(\w+)\s+ON\s+(\w+)/i)) {
      const match = line.match(/CREATE\s+(UNIQUE\s+)?INDEX\s+IF\s+NOT\s+EXISTS\s+(\w+)\s+ON\s+(\w+)\s*(.+)/i);
      if (match) {
        ...
        newLines.push(`DO $$`);
        newLines.push(`BEGIN`);
        newLines.push(`  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = '${tableName}') THEN`);
        newLines.push(`    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = '${tableName}' AND indexname = '${indexName}') THEN`);
        newLines.push(`      EXECUTE 'CREATE ${match[1] || ''}INDEX ${indexName} ON ${tableName} ${indexDef.replace(/'/g, "''")}';`);
