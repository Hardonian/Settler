...
test("buyer and operator surfaces do not advertise operational SCIM", () => {
  const surfaces = [
  ...
  for (const surface of surfaces) {
    const content = readFileSync(resolve(root, surface), "utf8");
    for (const claim of unsupportedClaims) {
      assert.doesNotMatch(content, claim, `${surface} contains unsupported claim ${claim}`);
    }
  }
});
