...
async function checkRLSPolicies(): Promise<{
...
async function checkEncryption(): Promise<{
  status: "pass" | "fail" | "warning";
  evidence: string;
}> {
  try {
...
async function checkMonitoring(): Promise<{
  status: "pass" | "fail" | "warning";
  evidence: string;
}> {
  try {
...
async function checkIncidentResponse(): Promise<{
  status: "pass" | "fail" | "warning";
  evidence: string;
}> {
  try {
    // Check if incident response process exists
    const incidentDoc = join(
      process.cwd(),
      "docs/internal/business/BUSINESS_READINESS_ASSESSMENT.md"
    );
...
async function runSOC2ReadinessCheck(): Promise<void> {
  ...
  results.push({
    ...SOC2_CONTROLS[0],
    status:
      rlsCheck.status === "pass" ? "pass" : rlsCheck.status === "warning" ? "warning" : "fail",
  ...
  results.push({
    ...SOC2_CONTROLS[4],
    status:
      monitoringCheck.status === "pass"
        ? "pass"
        : monitoringCheck.status === "warning"
          ? "warning"
          : "fail",
  ...
  results.push({
    ...SOC2_CONTROLS[6],
    status:
      incidentCheck.status === "pass"
        ? "pass"
        : incidentCheck.status === "warning"
          ? "warning"
          : "fail",
