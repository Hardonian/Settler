...
function addResult(name: string, status: DiagnosticResult['status'], message: string, details?: Record<string, unknown>) {
    ...
    if (error) {
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        addResult(
          'Supabase Connection',
          'warning',
          'Connected but api_keys table does not exist',
        ...
        addResult(
          'Supabase Connection',
          'warning',
          'Connected but RLS may be blocking queries',
          { error: error.message, code: error.code }
        );
      } else {
        addResult(
  ...
  try {
    ...
    if (errorMessage.includes('Can\'t reach database server') || errorMessage.includes('Connection')) {
      ...
      addResult(
        'Prisma Client',
        'warning',
        'Database connected but tables may be missing',
        { error: errorMessage }
      );
    } else {
      addResult(
...
async function checkDatabaseTables() {
  try {
    const criticalTables = ['billing_accounts', 'api_keys', 'receipts', 'usage_events', 'feature_flags'];
    const missing: string[] = [];
    const present: string[] = [];
    ...
    if (missing.length > 0) {
      addResult(
        'Database Tables',
        'warning',
        `Some tables may be missing: ${missing.join(', ')}`,
        { missing, present }
      );
    } else {
      addResult(
  ...
  try {
    ...
    if (error) {
      ...
      addResult(
        'RLS Policies',
        'pass',
        'RLS policies check skipped (requires direct DB access)',
  ...
  try {
    ...
    if (error && !error.message.includes('JWT')) {
      addResult(
        'Authentication',
        'warning',
        `Auth client works but error: ${error.message}`,
        { error: error.message }
      );
    } else {
      addResult(
        'Authentication',
        'pass',
        'Authentication client initialized correctly',
        { hasUser: !!user, note: 'No user expected in script context' }
      );
    }
  } catch (error) {
    addResult(
