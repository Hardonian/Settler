...
async function convertToWebP() {
  ...
  for (const { input, output } of imagesToConvert) {
    ...
    try {
      ...
      console.log(`  ✓ Created ${output} (${(stats.size / 1024).toFixed(1)}KB, ${savings}% smaller)\n`);
    } catch (error) {
      console.error(`  ✗ Error converting ${input}:`, error instanceof Error ? error.message : error);
    }
  }
