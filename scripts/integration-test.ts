...
async function integrationTest() {
  ...
  try {
      ...
      if (tenants.length > 0) {
        ...
        if (rowCount === 1) {
          ...
          if (insertedLog.length > 0) {
            try {
              const headers =
                typeof insertedLog[0].headers === "string"
              ...
              if (
                ...
                console.log(
                  "⚠️  PII sanitization check inconclusive (headers may be stored as JSONB)\n"
                );
              }
            } catch (error) {
          ...
          // Clean up
          await client.query(
            `DELETE FROM api_call_logs WHERE path = '/api/console/api-logs' AND method = 'POST'`
          );
        }
      }
    } catch (error) {
      console.log(
        `⚠️  Log insertion test: ${error instanceof Error ? error.message : "Unknown error"}\n`
      );
    }
    ...
    const compositeIndexes = indexes.filter(
      (idx: any) => idx.indexdef.includes(",") || idx.indexdef.includes("WHERE")
    );
