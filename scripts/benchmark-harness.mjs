...
const tsHarness = spawnSync('pnpm', ['exec', 'tsx', 'scripts/benchmark-harness.ts', '--sequential-count', String(sequentialCount), '--concurrent-count', String(concurrentCount), '--output', outputPath], {
  stdio: 'inherit',
  env: process.env,
});
...
const simulate = (index) => {
  const records = Array.from({ length: 20 }, (_, i) => ({ id: `${index}-${i}`, amount: i * 1.5 + index, ref: `INV-${index}-${i}` }));
  const digest = sha256(stableStringify(records));
  return { digest, matched: Math.floor(records.length * 0.95), unmatched: Math.ceil(records.length * 0.05) };
};
const p95 = (values) => {
