...
async function verifyRLSPolicies() {
  ...
  for (const table of tenantScopedTables) {
    try {
      ...
      addResult(`RLS: ${table}`, 'warning', `Could not verify RLS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
async function verifyDatabaseConstraints() {
  ...
  try {
    ...
    addResult('Database Constraints', 'warning', `Could not verify constraints: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function verifyDatabaseFunctions() {
  ...
  try {
    ...
    addResult('Database Functions', 'warning', `Could not verify functions: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function verifyRuntimeGuards() {
  ...
  if (fs.existsSync(billingEnforcementPath)) {
    ...
    addResult('Runtime Guard: billing-enforcement.ts', 'fail', 'Billing enforcement module missing');
  }
}
...
async function verifyAdapterPermissions() {
  ...
  const adapterRoutePath = path.join(
    process.cwd(),
  ...
  const hasPermissionCheck = 
    content.includes('checkIntegrationAccess') ||
    content.includes('requireAddOn');
