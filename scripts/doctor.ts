...
function addCheck(category: Category, name: string, status: CheckStatus, message: string, remediation?: string) {
...
function parseArgs$1(command: string): string {
  const res = spawnSync(command, ['--version'], { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
...
function execCommand(command: string, args: string[] = [], options: { silent?: boolean; timeout?: number } = {}): { success: boolean; output: string } {
  try {
...
async function checkDatabaseConnection() {
  ...
  try {
    ...
    if (result.success) {
      ...
      const testResult = execCommand('psql', [
        dbUrl,
        '-c', 'SELECT 1;',
        '-t',
      ...
      addCheck('database', 'Database Connection', 'warn', 'psql not available - cannot verify connection');
    }
  } catch (error) {
    addCheck(
      'database',
