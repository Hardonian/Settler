  ...
  private async crawlPage(relativeUrl: string): Promise<void> {
    ...
    try {
      ...
      if (result.status >= 200 && result.status < 400 && result.hasContent) {
        ...
        try {
          const safeFilename =
            relativeUrl.replace(/[^a-z0-9]/gi, "_").replace(/_+/g, "_") || "index";
          result.screenshotDesktop = await this.takeScreenshot(page, safeFilename, "desktop", {
            width: 1280,
            height: 720,
          });
          result.screenshotMobile = await this.takeScreenshot(page, safeFilename, "mobile", {
            width: 375,
            height: 667,
          });
        } catch (error) {
    ...
    try {
      const buttonLinks = await page.evaluate(() => {
        const buttons = Array.from(
          document.querySelectorAll('button[data-href], [role="button"][data-href]')
        );
