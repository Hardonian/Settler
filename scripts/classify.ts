...
function classifyFile(filePath: string, content: string | null): FileClassification {
  ...
  if (matchesPattern(relativePath, OSS_PUBLIC_PATHS)) {
    ...
    if (content) {
      const hasProprietaryImport = PROPRIETARY_IMPORT_PATTERNS.some((pattern) =>
...
function generateMarkdownSummary(report: ClassificationReport): string {
  ...
  const byClassification = report.files.reduce(
    (acc, file) => {
...
async function main() {
  ...
  if (jsonOnly) {
    ...
    console.log(
      `  SECRET_RISK: ${report.summary.secret_risk} ${report.summary.secret_risk > 0 ? "❌" : "✅"}`
    );
