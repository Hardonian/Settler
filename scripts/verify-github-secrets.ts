...
function categorizeSecret(name: string): string {
  ...
  if (name.includes('JWT') || name.includes('ENCRYPTION') || name.includes('SECRET')) return 'Security';
...
function main() {
  ...
  console.log(`Found ${secretsInWorkflows.size} unique secrets referenced in ${workflowFiles.length} workflows\n`);
  ...
  const nextPublicVars = Array.from(envVarsInCode.keys()).filter(v => v.startsWith('NEXT_PUBLIC_'));
  ...
  if (missingCritical.length > 0) {
    ...
    missingCritical.forEach(v => {
      ...
      console.log(`   ${inCode ? '⚠️' : '❓'} ${v}${inCode ? ' (used in codebase)' : ' (not found in codebase)'}`);
    });
  }
  ...
  if (nextPublicNotInSecrets.length > 0) {
    ...
    const important = nextPublicNotInSecrets.filter(v => 
      v.includes('SUPABASE') || v.includes('SITE_URL') || v.includes('APP_URL')
    );
    ...
    if (nextPublicNotInSecrets.length > important.length) {
      ...
      nextPublicNotInSecrets.filter(v => !important.includes(v)).forEach(v => console.log(`   ℹ️  ${v}`));
    }
  }
