...
interface RouteInfo {
  ...
  enforcementType?: 'requireActiveSubscription' | 'withBillingEnforcement' | 'withSubscriptionGate' | 'withUniversalBillingGate' | 'publicRoute' | 'freeRoute';
  lineNumbers: number[];
}
...
async function main() {
  ...
  const publicRoutes = results.filter(r => r.hasPublicMarker || r.enforcementType === 'publicRoute');
