...
class BuildSafetyValidator {
    ...
    for (const pkg of packages) {
      ...
      try {
        ...
        for (const [scriptName, scriptCommand] of Object.entries(scripts)) {
          ...
          const scriptMatches = scriptCommand.match(
            /(?:node|tsx|bash|sh)\s+(?:\.\.\/)*scripts\/([^\s"']+)/g
          );
          if (scriptMatches) {
            for (const match of scriptMatches) {
              ...
              if (!existsSync(fullPath)) {
                ...
                const hasFallback =
                  typeof scriptCommand === "string" &&
                  (scriptCommand.includes("||") || scriptCommand.includes("&& echo"));
      ...
      try {
        ...
        if (scripts.lint || scripts["lint:fix"]) {
          const eslintConfigPath =
            join(packagesDir, pkg, ".eslintrc.js") ||
        ...
        if (config.extends) {
          ...
          for (const extend of extendsList) {
            ...
            if (
              ...
              extend.startsWith("plugin:") ||
              extend.startsWith("next/")
            ) {
              continue;
            }
  ...
  private checkESLintExtendsDependencies(): void {
    ...
    for (const pkg of packages) {
      const packageJsonPath = join(packagesDir, pkg, "package.json");
      const eslintConfigPath =
        join(packagesDir, pkg, ".eslintrc.js") || join(packagesDir, pkg, ".eslintrc.json");
      ...
      try {
        ...
        const extendsRoot =
          eslintConfigContent.includes("../../.eslintrc.js") ||
    ...
    for (const pkg of packages) {
      ...
      try {
        ...
        for (const scriptName of buildScripts) {
          ...
          const hasExistenceCheck =
            script.includes("test -f") || script.includes("existsSync") || script.match(/\[ -f/);
