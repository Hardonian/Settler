...
${ENV_MAPPINGS.filter((m) => m.required)
  .map(
    (m) => `- **${m.vercelKey}** (${m.isPublic ? "Public" : "Private"})
...
${ENV_MAPPINGS.map(
  (m) =>
    `| \`${m.githubSecret}\` | \`${m.vercelKey}\` | ${m.isPublic ? "Yes" : "No"} | ${m.required ? "Yes" : "No"} |`
).join("\n")}
...
function main() {
  ...
  console.log(
    `   Public (NEXT_PUBLIC_) variables: ${ENV_MAPPINGS.filter((m) => m.isPublic).length}`
  );
