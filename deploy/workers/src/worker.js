...
async function handleRequest(request, env) {
  ...
  if (path === '/api/v1/jobs' && method === 'POST') {
    ...
    ).bind(source_type, target_type, JSON.stringify(source_config || {}), JSON.stringify(target_config || {})).run();

    const response = json({ job_id: result.meta.last_row_id, source_type, target_type, status: 'pending' });
    Object.entries(cors).forEach(([k, v]) => response.headers.set(k, v));
    return response;
  }
  ...
  async scheduled(event, env) {
    if (event.cron === "0 3 * * *") {
      ...
      for (const job of pending.results) {
        ...
        ).bind(JSON.stringify({ matched: 42, diffs: 3 }), job.id).run();
