// Edge Function: integration-sync-stripe
// Purpose: Sync Stripe data and log usage events
...
serve(async (req) => {
  ...
  try {
    ...
    // Log usage event for Stripe sync
    const { data: eventId, error: logError } = await supabaseClient.rpc("log_usage_event", {
      ...
      p_integration_id: "stripe",
      p_unit: "sync",
      p_metadata: { integration: "stripe", sync_type: "full" },
    });
    ...
    return new Response(
      JSON.stringify({
        ...
        event_id: eventId,
        integration: "stripe",
        message: "Stripe sync completed and usage logged",
      }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
