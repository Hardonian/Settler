...
test.describe("Receipt Console", () => {
  ...
  test("should handle authentication errors gracefully", async ({ page, context }) => {
    ...
    const url = page.url();
    const isSignInPage =
      url.includes("/signup") || url.includes("/signin") || url.includes("/login");
    const hasAuthError = await page
      .getByText(/sign in|authentication|unauthorized/i)
      .isVisible()
      .catch(() => false);
  ...
  test("should display receipt table when receipts exist", async ({ page }) => {
    ...
    await page.waitForResponse(
      (response) => response.url().includes("/api/console/receipts") && response.status() === 200
    );
