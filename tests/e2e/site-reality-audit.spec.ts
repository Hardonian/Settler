...
test.describe('Site Reality Audit', () => {
  test('critical public routes load without 500', async ({ page }) => {
    ...
    if (failures.length > 0) {
      ...
      throw new Error(`${failures.length} route(s) failed: ${failures.map(f => `${f.route} (${f.status || f.error})`).join(', ')}`);
    }
  });
  ...
  test('navigation links work', async ({ page }) => {
    ...
    for (const link of links.slice(0, 20)) { // Limit to first 20 to avoid timeout
    ...
    if (failures.length > 0) {
      throw new Error(`${failures.length} navigation link(s) returned 500: ${failures.map(f => `${f.href} (${f.status})`).join(', ')}`);
    }
  });
