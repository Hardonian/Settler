...
test.describe("End-to-End Operational Lifecycle Reality Pass", () => {
  ...
  test("Executes a full run, identifies exceptions, resolves them, and verifies audit provenance", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("Status: Completed with Exceptions")).toBeVisible({
      timeout: 15000,
    });
    ...
    const updatedExceptionCount = await page
      .locator('[data-testid="unresolved-exception-count"]')
      .textContent();
  ...
  test("Hostile Condition: Degraded worker state blocks ingestion truthfully", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        "Action Blocked: Reconciliation Workers are currently disabled in Infrastructure Settings"
      )
    ).toBeVisible();
  });

  test("Resilience: Stale Run Reaper identifies and transitions stalled processing runs", async ({
    request,
    page,
  }) => {
    ...
    await expect(
      page.getByText("System recovered orphaned run after worker timeout.")
    ).toBeVisible();
  });

  test("Bulk-Audit Semantics: Resolving multiple exceptions produces a single verifiable batch audit trail", async ({
    page,
  }) => {
