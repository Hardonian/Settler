...
const jobPostings: JobPosting[] = [
  {
    ...
    description:
      "Looking for someone to build our reconciliation processes from scratch. Experience with high-volume transaction matching required.",
  ...
  },
  {
    ...
    description:
      "Manage payment reconciliation across multiple processors including Stripe and PayPal.",
    location: "Remote",
    postedAt: "1 day ago",
    source: "AngelList",
    url: "https://angel.co/...",
  ...
  },
  {
    ...
    description:
      "Build finance function including automated reconciliation for marketplace transactions.",
    location: "Austin, TX",
...
function scoreJobPosting(posting: JobPosting): JobPosting {
  ...
  // Seniority bonus
  if (
    posting.title.toLowerCase().includes("director") ||
...
fs.writeFileSync(`${outputArg}/jobs-${timestamp}.json`, JSON.stringify(filteredPostings, null, 2));

console.log(
  `✅ Saved ${filteredPostings.length} job postings to ${outputArg}/jobs-${timestamp}.json`
);
