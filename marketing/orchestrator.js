...
async function runWithApproval() {
  ...
  if (fs.existsSync(blogDir)) {
    ...
    blogs.forEach((blog) => {
      ...
      execSync(
        `node ${APPROVAL_WORKFLOW} --action=submit --type=blog --title="${title}" --content="${blogDir}/${blog}"`
      );
    });
  }
  ...
  if (fs.existsSync(socialDir)) {
    ...
    socials.forEach((social) => {
      ...
      const platform = content.includes("TWITTER")
        ? "twitter"
      ...
      execSync(
        `node ${APPROVAL_WORKFLOW} --action=submit --type=social --title="${title}" --content="${social}" --platform=${platform}`
      );
    });
  }
