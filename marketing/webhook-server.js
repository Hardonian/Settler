...
class WebhookServer {
  ...
  handleSlack(data, res) {
    ...
    if (data.command === '/approve') {
      ...
      execSync(`cd ${MARKETING_DIR} && node approval-workflow.js --action=approve --id=${data.text}`);
  ...
  handleApproval(data, res) {
    ...
    try {
      execSync(`cd ${MARKETING_DIR} && node approval-workflow.js --action=${action} --id=${id} --approver=${approver}`);
  ...
  handleTrigger(data, res) {
    ...
    switch (type) {
      ...
      case 'publish-social':
        ...
        execSync(`cd ${MARKETING_DIR} && node integrations/social-publisher.js publish --platform=${platform} --content="${content}"`);
