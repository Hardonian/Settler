...
debugCommand
    ...
    try {
        ...
        if (typeof config !== 'object' || config === null) {
            console.error(chalk_1.default.red("❌ Validation failed: Config must be an object"));
            process.exit(1);
        }
        ...
        if (!configObj.source || typeof configObj.source !== 'object' || !configObj.source.adapter) {
            errors.push("Missing required field: source.adapter");
        }
        if (!configObj.target || typeof configObj.target !== 'object' || !configObj.target.adapter) {
            errors.push("Missing required field: target.adapter");
        }
        if (!configObj.rules || typeof configObj.rules !== 'object' || !Array.isArray(configObj.rules.matching)) {
        ...
        console.log(chalk_1.default.gray(`   Name: ${configObj.name}`));
        console.log(chalk_1.default.gray(`   Source: ${configObj.source.adapter}`));
        console.log(chalk_1.default.gray(`   Target: ${configObj.target.adapter}`));
        const rules = configObj.rules;
        console.log(chalk_1.default.gray(`   Matching rules: ${rules.matching?.length || 0}`));
    }
    catch (error) {
        console.error(chalk_1.default.red(`Error: ${error instanceof Error ? error.message : "Unknown error"}`));
        process.exit(1);
