...
class SecurityAgent {
  ...
  async scan(
    type: "vulnerabilities" | "secrets" | "rls" | "compliance" | "all"
    ...
    if (this.config.githubToken) {
      try {
        const res = await fetch(
          `https://api.github.com/repos/${this.config.repo}/dependabot/alerts`,
          {
            headers: {
    ...
    const secretPatterns = [
      {
        ...
        message: "Potential API key in code",
      },
...
const scanArg =
  (args.find((a) => a.startsWith("--scan="))?.split("=")[1] as
    | "vulnerabilities"
    | "secrets"
    | "rls"
    | "compliance"
