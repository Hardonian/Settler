    ...
    await fetch(this.config.slackWebhook, {
      ...
      body: JSON.stringify({
        ...
        blocks: [
          ...
          alert.metadata ? {
            type: 'section',
            text: { type: 'mrkdwn', text: `*Details:*n${JSON.stringify(alert.metadata, null, 2)}` },
          } : undefined,
        ].filter(Boolean),
      }),
    });
...
const priorityArg = args.find(a => a.startsWith('--priority='))?.split('=')[1] as Alert['priority'] || 'medium';
