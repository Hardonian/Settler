...
import { getTenantCacheIndexKey, invalidateTenantCacheKeys } from "./tenant-cache";
...
export async function invalidateTenantCache(tenantId: string): Promise<void> {
  await invalidateTenantCacheKeys(tenantId);

  const patterns = [
    cacheKey("tenant", tenantId),
    cacheKey("tenant", tenantId, "*"),
    getTenantCacheIndexKey(tenantId),
    cacheKey("tenant", "lookup", "id", tenantId),
    // Legacy cache namespaces (defense-in-depth)
    cacheKey("tenants", tenantId),
    cacheKey("tenants", tenantId, "*"),
    cacheKey("tenantById", tenantId),
    cacheKey("tenantBySlug", "*"),
    cacheKey("tenantByDomain", "*"),
