...
function createPrintfFormat() {
    return winston_1.default.format.printf(function (info) {
        ...
        var requestInfo = request_id && typeof request_id === "string" ? "[req=".concat(request_id.substring(0, 8), "]") : "";
        var traceInfo = trace_id && typeof trace_id === "string" ? "[trace=".concat(trace_id.substring(0, 8), "]") : "";
        var spanInfo = span_id && typeof span_id === "string" ? "[span=".concat(span_id.substring(0, 8), "]") : "";
        var tenantInfo = tenant_id && typeof tenant_id === "string" ? "[tenant=".concat(tenant_id, "]") : "";
        var userInfo = user_id && typeof user_id === "string" ? "[user=".concat(user_id, "]") : "";
        var executionInfo = execution_id && typeof execution_id === "string"
        ...
        return "".concat(timestampStr, " [").concat(level, "]").concat(requestInfo).concat(traceInfo).concat(spanInfo).concat(tenantInfo).concat(userInfo).concat(executionInfo, ": ").concat(messageStr, " ").concat(metaStr);
    });
}
var logFormat = winston_1.default.format.combine(contextFormat(), winston_1.default.format.timestamp(), winston_1.default.format.errors({ stack: true }), winston_1.default.format.json());
...
function logError(message, error, meta) {
    exports.logger.error(message, __assign(__assign({}, (0, redaction_1.redact)(meta)), (error instanceof Error
...
function logBusinessEvent(event, meta) {
    exports.logger.info("business_event:".concat(event), __assign({ event_type: event }, (0, redaction_1.redact)(meta)));
}
function logPerformance(operation, durationMs, meta) {
    exports.logger.info("performance:".concat(operation), __assign({ operation: operation, duration_ms: durationMs }, (0, redaction_1.redact)(meta)));
}
