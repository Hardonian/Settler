...
export interface TelemetryExecutionRecord {
  executionId: string;
  tenantId: string;
  traceId: string;
  timestamp: string;
...
export interface PlatformOverview {
  telemetry: {
    ...
    failureTrends: Record<string, number>;
  ...
  analytics: {
    ...
    mttrMinutes: number;
    failureRecurrenceClusters: Array<{ failureClass: string; occurrences: number }>;
    apiEndpointAdoption: Array<{ endpoint: string; count: number }>;
    latencyTrend: "improving" | "degrading" | "stable";
    infrastructureTrend: "improving" | "degrading" | "stable";
  };
  costs: {
    perExecutionAverageUsd: number;
    totalUsd: number;
    byTenantUsd: number;
  };
  autonomousOperations: {
    recommendations: Array<{ type: string; reason: string; evidence: string }>;
    controlledAutoRemediation: Array<{ action: string; reason: string; maxRetries: number }>;
  };
  leaderboard: Array<{
    workflowKey: string;
...
function trend(before: number, after: number): "improving" | "degrading" | "stable" {
  const delta = after - before;
  if (Math.abs(delta) < Math.max(5, before * 0.05)) return "stable";
  return delta > 0 ? "degrading" : "improving";
}

export function buildPlatformOverview(records: TelemetryExecutionRecord[]): PlatformOverview {
  ...
  const endpointAdoptionMap: Record<string, number> = {};
  records.forEach((r) => {
  ...
  const workflowStats = new Map<
    string,
    { total: number; failed: number; latency: number; resources: number; compute: number }
  >();
  records.forEach((r) => {
  ...
  const recommendations: PlatformOverview["autonomousOperations"]["recommendations"] = [];
  ...
  const controlledAutoRemediation: PlatformOverview["autonomousOperations"]["controlledAutoRemediation"] =
    [];
...
export async function loadTenantTelemetry(
  tenantId: string,
  days: number
  ...
  const records = await queryWithTenant<{
    ...
    ORDER BY COALESCE(e.started_at, e.created_at) ASC`,
    [tenantId, days]
  );
  ...
  return records.map((r) => ({
    ...
    status: (r.status as "success" | "failed" | "running") || "running",
