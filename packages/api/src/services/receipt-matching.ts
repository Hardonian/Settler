  ...
  try {
    ...
    for (const receipt of receipts) {
      ...
      if (bestMatch && bestMatch.confidenceScore > 0.5) {
        // Create link
        await query(
          `INSERT INTO receipt_transaction_links (
    ...
    logError("Failed to match receipts", error, { tenantId, reconciliationRunId });
    throw error;
  }
}
...
function calculateMatch(
  receipt: { amount: number; date: Date; vendor?: string; description?: string },
  ...
  const dateDiff = Math.abs(
    receipt.date.getTime() - transaction.date.getTime()
  );
  ...
  if (receipt.description && transaction.description) {
    const similarity = stringSimilarity(
      receipt.description.toLowerCase(),
  ...
  if (receipt.vendor && transaction.description) {
    if (
      transaction.description.toLowerCase().includes(receipt.vendor.toLowerCase())
    ) {
      score += 0.1;
...
export async function verifyReceiptLink(
  ...
  try {
    await query(
      `UPDATE receipt_transaction_links
       SET verified = true, verified_by = $1, verified_at = now()
       WHERE id = $2 AND tenant_id = $3`,
      [verifiedBy, linkId, tenantId]
    );

    logInfo("Receipt link verified", { linkId, tenantId, verifiedBy });
  } catch (error) {
    logError("Failed to verify receipt link", error, { linkId, tenantId });
    throw error;
  }
}
