...
export class PredictiveOps {
  ...
  private async predictMappingVolatility(): Promise<FailurePrediction[]> {
    ...
    // Batch fetch updates grouped by name
    const mappingNames = Array.from(new Set(mappings.map((m) => m.name)));
    ...
    // Group the updates by name and limit to top 10 to match the previous take: 10
    const updatesByName = allUpdates.reduce(
      (acc, update) => {
        if (!acc[update.name]) acc[update.name] = [];
        if (acc[update.name].length < 10) acc[update.name].push(update);
        return acc;
      },
    ...
    for (const mapping of mappings) {
      const updates = updatesByName[mapping.name] || [];
