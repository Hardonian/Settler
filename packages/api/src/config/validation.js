...
function isBuildContext() {
    ...
    if (typeof process !== "undefined" && process.argv) {
        var isBuildCommand = process.argv.some(function (arg) { return arg.includes("build") || arg.includes("next build") || arg.includes("turbo build"); });
...
var isBuild = isBuildContext();
var BUILD_PLACEHOLDER = "build-placeholder-not-used-at-runtime";
exports.env = (0, envalid_1.cleanEnv)(process.env, {
    ...
    DB_PASSWORD: (0, envalid_1.str)(__assign(__assign({}, (isBuild ? { default: BUILD_PLACEHOLDER } : {})), { devDefault: "postgres" })),
    ...
    // JWT Configuration
    // Runtime-only: provide default during build, will be validated at runtime
    JWT_SECRET: (0, envalid_1.str)(__assign(__assign({}, (isBuild ? { default: BUILD_PLACEHOLDER } : {})), { devDefault: "dev-secret-change-in-production", desc: "Secret key for JWT token signing" })),
    ...
    // Runtime-only: provide default during build, will be validated at runtime
    ENCRYPTION_KEY: (0, envalid_1.str)(__assign(__assign({}, (isBuild ? { default: BUILD_PLACEHOLDER } : {})), { devDefault: "dev-encryption-key-32-chars-long!!", desc: "32-byte key for AES-256-GCM encryption" })),
    // Rate Limiting
...
if (!isBuild && (exports.env.NODE_ENV === "production" || exports.env.NODE_ENV === "preview")) {
    ...
    if (exports.env.ENCRYPTION_KEY === BUILD_PLACEHOLDER ||
        ...
        throw new Error("ENCRYPTION_KEY must be exactly 32 characters in ".concat(exports.env.NODE_ENV, ". Current value: ").concat(exports.env.ENCRYPTION_KEY === BUILD_PLACEHOLDER ? "build placeholder (not set)" : "invalid length"));
    }
    if (exports.env.JWT_SECRET === BUILD_PLACEHOLDER ||
        !exports.env.JWT_SECRET ||
        exports.env.JWT_SECRET === "dev-secret-change-in-production") {
        throw new Error("JWT_SECRET must be set to a secure random value in ".concat(exports.env.NODE_ENV, ". Current value: ").concat(exports.env.JWT_SECRET === BUILD_PLACEHOLDER ? "build placeholder (not set)" : "dev secret"));
    }
    if (exports.env.DB_PASSWORD === BUILD_PLACEHOLDER) {
        throw new Error("DB_PASSWORD must be set in ".concat(exports.env.NODE_ENV, ". Current value: build placeholder (not set)"));
    }
    if (exports.env.ALLOWED_ORIGINS === "*") {
        console.warn("WARNING: CORS allows all origins in ".concat(exports.env.NODE_ENV, ". Consider restricting ALLOWED_ORIGINS."), {
...
exports.validatedConfig = {
    ...
    allowedOrigins: exports.env.ALLOWED_ORIGINS.split(",").map(function (o) { return o.trim(); }),
    logging: {
