...
export const ENV_VAR_SCHEMA: EnvVarSpec[] = [
  ...
  {
    name: "JOBFORGE_INTEGRATION_ENABLED",
    description: "Enable JobForge integration (admin + CLI tooling)",
    type: "boolean",
    required: false,
    scope: "runtime",
    exposure: "server-only",
    ...
    secret: false,
    platforms: ["github", "vercel", "local"],
    notes: "Defaults to disabled to prevent side effects. Enable explicitly when ready.",
  },
  {
    name: "JOBFORGE_BUNDLE_EXECUTION_ENABLED",
    description: "Allow JobForge bundle execution requests",
    type: "boolean",
    required: false,
    scope: "runtime",
    exposure: "server-only",
    ...
    secret: false,
    platforms: ["github", "vercel", "local"],
    notes: "Gates bundle execution requests in admin + CLI tooling.",
  },
  ...
  {
    name: "JEV_MODE",
    description: "TypeSafe AI Jev rollout mode for exception decision intelligence",
    type: "string",
    required: false,
    scope: "runtime",
    exposure: "server-only",
    ...
    environments: ["local", "development", "preview", "staging", "production"],
    defaultValue: "off",
    format: "off | shadow | recommend",
    validator: (value) => ["off", "shadow", "recommend"].includes(value),
    secret: false,
    platforms: ["github", "vercel", "local"],
  },
  {
    name: "TYPESAFE_API_KEY",
    description: "TypeSafe AI server-side API key",
    type: "string",
    required: false,
    scope: "runtime",
    exposure: "server-only",
    ...
    secret: true,
    platforms: ["github", "vercel", "local"],
    notes: "Required only when JEV_MODE is shadow or recommend. Never expose to the browser.",
  },
  {
    name: "TYPESAFE_BASE_URL",
    description: "Optional TypeSafe AI API base URL override",
    type: "url",
    required: false,
    scope: "runtime",
    exposure: "server-only",
    ...
    format: "HTTPS URL",
    secret: false,
    platforms: ["vercel", "local"],
  },
  {
    name: "TYPESAFE_DEFAULT_MODEL",
    description: "TypeSafe AI model used for System One decisions",
    type: "string",
    required: false,
    scope: "runtime",
    exposure: "server-only",
  ...
  ...[
    ["JEV_TIMEOUT_MS", "Per-attempt Jev request timeout", 3000],
    ["JEV_MAX_RETRIES", "Maximum Jev retries after the initial request", 1],
    ["JEV_BATCH_SIZE", "Maximum de-identified exceptions per Jev request", 10],
    ["JEV_CIRCUIT_FAILURE_THRESHOLD", "Failures before the Jev circuit opens", 3],
    ["JEV_CIRCUIT_RESET_MS", "Jev circuit breaker cooldown", 60000],
  ].map(([name, description, defaultValue]): EnvVarSpec => ({
    name: name as string,
  ...
  {
    name: "JEV_MIN_CONFIDENCE",
    description: "Minimum Jev confidence required to apply a recommendation",
    type: "number",
    required: false,
    scope: "runtime",
    exposure: "server-only",
  ...
  {
    name: "JEV_TENANT_ALLOWLIST",
    description: "Comma-separated tenant UUIDs allowed to use Jev; empty disables all tenants",
    type: "string",
    required: false,
    scope: "runtime",
    exposure: "server-only",
