...
function executeMigration(migrationPath, migrationName, useSupabase) {
    return __awaiter(this, void 0, void 0, function () {
        var result, migrationSQL, statements, connectionString, supabaseUrl, hostMatch, host, pool, _i, statements_1, statement, error_1, errorMessage, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    (0, logger_1.logInfo)("Running migration: ".concat(migrationName), { statements: statements.length });
                    ...
                    if (!connectionString && useSupabase) {
                        ...
                        if (process.env.SUPABASE_DB_PASSWORD) {
                            ...
                            host = hostMatch ? "".concat(hostMatch[1], ".supabase.co") : config_1.config.database.host;
                            connectionString = "postgresql://postgres:".concat(process.env.SUPABASE_DB_PASSWORD, "@").concat(host, ":").concat(config_1.config.database.port || 5432, "/postgres");
                        }
                        else {
                            // Fallback to config-based connection
                            connectionString = "postgresql://".concat(config_1.config.database.user, ":").concat(config_1.config.database.password, "@").concat(config_1.config.database.host, ":").concat(config_1.config.database.port, "/").concat(config_1.config.database.name);
                        }
                    }
                    if (!connectionString) {
                        // Fallback to config-based connection
                ...
                case 8:
                    _a.sent();
                    result.success = true;
                    (0, logger_1.logInfo)("Migration completed: ".concat(migrationName), { statements: result.statementsExecuted });
                    return [3 /*break*/, 10];
                case 9:
                    error_2 = _a.sent();
...
function initializeSupabaseExtensions() {
    return __awaiter(this, void 0, void 0, function () {
        var extensions, connectionString, useSupabase, supabaseUrl, hostMatch, projectRef, region, host, pool, _i, extensions_1, ext, error_3, errorMessage, error_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 8, , 9]);
                    ...
                    if (!connectionString && useSupabase) {
                        if (process.env.SUPABASE_DB_PASSWORD) {
                            ...
                            if (hostMatch) {
                                ...
                                connectionString = "postgresql://postgres.".concat(projectRef, ":").concat(process.env.SUPABASE_DB_PASSWORD, "@").concat(host, ":5432/postgres");
                            }
                            else {
                                connectionString = "postgresql://".concat(config_1.config.database.user, ":").concat(config_1.config.database.password, "@").concat(config_1.config.database.host, ":").concat(config_1.config.database.port, "/").concat(config_1.config.database.name);
                            }
...
function runMigrations() {
    return __awaiter(this, void 0, void 0, function () {
        var results, useSupabase, migrations, migrationsDir, _i, migrations_1, migrationFile, migrationPath, result, error_5, successCount, failCount;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    results = [];
                ...
                case 5:
                    ...
                    results.push(result);
                    if (!result.success) {
                        (0, logger_1.logError)("Migration ".concat(migrationFile, " failed, stopping migration process"));
                        return [3 /*break*/, 8];
                    }
                    return [3 /*break*/, 7];
                case 6:
