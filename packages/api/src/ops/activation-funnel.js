...
function emitLifecycleEvent(eventType, params) {
    return __awaiter(this, void 0, void 0, function () {
        var userId, tenantId, billingAccountId, _a, properties, finalBillingAccountId, account, tenant, user, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 8, , 9]);
                    userId = params.userId, tenantId = params.tenantId, billingAccountId = params.billingAccountId, _a = params.properties, properties = _a === void 0 ? {} : _a;
                    finalBillingAccountId = billingAccountId;
                    if (!(!finalBillingAccountId && userId)) return [3 /*break*/, 2];
                    return [4 /*yield*/, prisma_1.prisma.billingAccount.findFirst({
                            where: { userId: userId },
                            select: { id: true },
                        })];
                case 1:
                ...
                case 2:
                    ...
                    return [4 /*yield*/, prisma_1.prisma.tenant.findUnique({
                            where: { id: tenantId },
                            select: { billingAccountId: true },
                        })];
                case 3:
                    tenant = _b.sent();
                    finalBillingAccountId = (tenant === null || tenant === void 0 ? void 0 : tenant.billingAccountId) || undefined;
                    _b.label = 4;
                case 4:
                ...
                case 5:
                    ...
                    if (!user) {
                        ...
                        Promise.resolve().then(function () { return require("../utils/logger"); }).then(function (_a) {
                            var logWarn = _a.logWarn;
                            logWarn("Cannot emit lifecycle event ".concat(eventType, ": no billing account found"));
                        })
                            .catch(function () {
                ...
                case 6:
                    if (!finalBillingAccountId) {
                        ...
                        Promise.resolve().then(function () { return require("../utils/logger"); }).then(function (_a) {
                            var logWarn = _a.logWarn;
                            logWarn("Cannot emit lifecycle event ".concat(eventType, ": no billing account ID available"));
                        })
                            .catch(function () {
                            // Silent fail if logger unavailable
                        });
                    ...
                    return [4 /*yield*/, prisma_1.prisma.usageEvent.create({
                            data: {
...
function getActivationFunnelMetrics(params) {
    return __awaiter(this, void 0, void 0, function () {
        var startDate, endDate, tenantId, where, _a, signups, tenantsCreated, providersConnected, firstRecons, exceptionsCreated, exceptionsResolved, checkoutsStarted, checkoutsCompleted, paymentsFailed, subscriptionsCanceled, conversionRates;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    startDate = params.startDate, endDate = params.endDate, tenantId = params.tenantId;
                    where = {
                        timestamp: {
                            gte: startDate,
                            lt: endDate,
                    ...
                    return [4 /*yield*/, Promise.all([
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.USER_SIGNED_UP }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.TENANT_CREATED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.PROVIDER_CONNECTED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.RECON_FIRST_RUN }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.RECON_EXCEPTION_CREATED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.RECON_EXCEPTION_RESOLVED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.BILLING_CHECKOUT_STARTED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.BILLING_CHECKOUT_COMPLETED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.BILLING_PAYMENT_FAILED }),
                            }),
                            prisma_1.prisma.usageEvent.count({
                                where: __assign(__assign({}, where), { eventType: LifecycleEventType.BILLING_SUBSCRIPTION_CANCELED }),
                            }),
                        ])];
                case 1:
                    _a = _b.sent(), signups = _a[0], tenantsCreated = _a[1], providersConnected = _a[2], firstRecons = _a[3], exceptionsCreated = _a[4], exceptionsResolved = _a[5], checkoutsStarted = _a[6], checkoutsCompleted = _a[7], paymentsFailed = _a[8], subscriptionsCanceled = _a[9];
                    conversionRates = {
