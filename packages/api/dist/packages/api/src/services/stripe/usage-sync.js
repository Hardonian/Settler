...
exports.StripeUsageSync = void 0;
const stripe_1 = __importDefault(require("stripe"));
const logger_1 = require("../../utils/logger");
class StripeUsageSync {
    prisma;
    stripe;
    constructor(prisma, stripeKey) {
        this.prisma = prisma;
     ...
     */
    async syncUsageToStripe(billingAccountId, startDate, endDate) {
        try {
            ...
            if (!billingAccount?.stripeCustomerId) {
                (0, logger_1.logError)('No Stripe customer ID found', { billingAccountId });
                return;
            }
            ...
            for (const subscription of billingAccount.subscriptions) {
                ...
                for (const [eventType, usage] of usageByType.entries()) {
                    ...
                    const matchingItem = subscriptionItems.data.find(item => item.metadata?.eventType === eventType);
                    if (matchingItem && usage.quantity > 0) {
                        ...
                        (0, logger_1.logInfo)('Usage synced to Stripe', {
                            billingAccountId,
                            eventType,
            ...
            (0, logger_1.logError)('Failed to sync usage to Stripe', { error, billingAccountId });
            throw error;
        }
    }
    /**
     ...
     */
    async syncCurrentPeriod(billingAccountId) {
        const billingAccount = await this.prisma.billingAccount.findUnique({
            where: { id: billingAccountId },
            include: {
                subscriptions: {
...
exports.StripeUsageSync = StripeUsageSync;
