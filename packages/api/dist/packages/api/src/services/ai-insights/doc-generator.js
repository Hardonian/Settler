...
exports.generateRouteDocs = generateRouteDocs;
exports.generateMarkdownDocs = generateMarkdownDocs;
exports.saveRouteDocs = saveRouteDocs;
    ...
    try {
        ...
        for (const file of routeFiles) {
            try {
                ...
                (0, logger_1.logError)(`Failed to parse route file: ${file}`, error);
            }
        }
        ...
        (0, logger_1.logError)("Failed to generate route docs", error);
        return {
            totalRoutes: 0,
            documentedRoutes: 0,
...
async function findRouteFiles(dir, files = []) {
    try {
        ...
        (0, logger_1.logError)(`Failed to read directory: ${dir}`, error);
        return files;
    }
}
/**
...
async function parseRouteFile(filePath) {
    try {
        ...
        (0, logger_1.logError)(`Failed to parse route file: ${filePath}`, error);
        return [];
    }
}
/**
 ...
 */
async function generateMarkdownDocs(report) {
    const outputLines = [];
 ...
 */
async function saveRouteDocs(outputPath, report) {
    try {
        const markdown = await generateMarkdownDocs(report);
        await fs.writeFile(outputPath, markdown, "utf-8");
        (0, logger_1.logInfo)("Route documentation saved", { outputPath, totalRoutes: report.totalRoutes });
    }
    catch (error) {
        (0, logger_1.logError)("Failed to save route docs", error);
        throw error;
    }
}
