...
exports.DEFAULT_GOLDEN_FIXTURES = void 0;
exports.runGoldenDeterminismTest = runGoldenDeterminismTest;
exports.runAllGoldenTests = runAllGoldenTests;
exports.generateSyntheticData = generateSyntheticData;
exports.runPressureTest = runPressureTest;
exports.runPressureTestSuite = runPressureTestSuite;
 ...
 */
exports.DEFAULT_GOLDEN_FIXTURES = [
    {
 ...
 */
async function runGoldenDeterminismTest(fixture, tenantId = 'test-tenant') {
    (0, logger_1.logInfo)('Running golden determinism test', { fixtureName: fixture.name });
    // Create mock snapshot
    const createMockSnapshot = (runId) => ({
        id: runId,
        tenant_id: tenantId,
    ...
    const engine1 = new deterministic_matcher_1.DeterministicMatchingEngine({
    ...
    const engine2 = new deterministic_matcher_1.DeterministicMatchingEngine({
...
async function runAllGoldenTests() {
    const results = [];
    for (const fixture of exports.DEFAULT_GOLDEN_FIXTURES) {
        try {
            const result = await runGoldenDeterminismTest(fixture);
            results.push(result);
        }
        catch (error) {
            (0, logger_1.logError)('Golden test failed with error', error, { fixtureName: fixture.name });
            results.push({
                fixture_name: fixture.name,
                passed: false,
    ...
    (0, logger_1.logInfo)('Golden determinism tests complete', { total: results.length, passed, failed });
    return {
 ...
 * Run pressure test
 */
async function runPressureTest(recordCount, concurrency, tenantId = 'test-tenant') {
    (0, logger_1.logInfo)('Running pressure test', { recordCount, concurrency });
 ...
 */
async function runPressureTestSuite() {
 ...
 */
function computeResultsFingerprint(matches) {
    const sortedMatches = [...matches].sort((a, b) => a.stable_match_id.localeCompare(b.stable_match_id));
    const data = sortedMatches.map(m => ({
        stable_match_id: m.stable_match_id,
        confidence: m.confidence_score,
    }));
    return (0, node_crypto_1.createHash)('sha256')
        .update((0, canonical_input_1.stableStringify)(data))
        .digest('hex');
}
/**
...
async function validateDeterminism() {
    (0, logger_1.logInfo)('Starting determinism validation');
    ...
    (0, logger_1.logInfo)('Determinism validation complete', {
        success,
