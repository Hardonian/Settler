...
exports.buildImportWorkbenchPreview = buildImportWorkbenchPreview;
...
const REQUIRED_MAPPING_FIELDS = ["amount", "date"];
...
function withRemediation(diagnostic) {
...
function parseCSV(csvContent) {
    try {
        ...
        if (duplicates.length > 0) {
            throw new CSVParserError(`CSV contains duplicate headers: ${[...new Set(duplicates)].join(", ")}`);
        }
...
function normalizeAmountWithReason(rawValue) {
...
function parseDateWithReason(rawValue) {
...
function buildImportWorkbenchPreview(params) {
    ...
    for (let index = 0; index < rows.length; index++) {
        ...
        const normalized = types_1.NormalizedTransactionSchema.parse({
    ...
    const qualityGates = [
        ...
        },
        {
            ...
            severity: "warning",
            passed: attemptedRows === 0 ? false : normalizedRows / attemptedRows >= profileGates.normalizationThreshold,
            message: attemptedRows === 0
        ...
        },
        {
            ...
            message: invalidCurrencyCount === 0
