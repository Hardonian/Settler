...
exports.formatWeeklyReportMarkdown = formatWeeklyReportMarkdown;
exports.saveWeeklyReport = saveWeeklyReport;
const client_1 = require("@prisma/client");
const promises_1 = require("fs/promises");
const path_1 = require("path");
const cost_baselines_1 = require("../../../../../ops/cost_baselines");
const COST_BASELINES = cost_baselines_1.COST_BASELINES ?? {
    vercel: {
...
async function generateWeeklyReport() {
    ...
    const previousWeekRevenue = previousWeekUsageRevenue.reduce((sum, agg) => sum + Number(agg.estimatedCost || 0), 0);
    ...
    // Cost proxy
    const estimatedInfraBaseline = (apiCalls * COST_BASELINES.vercel.serverlessRequest.costPerUnit) +
...
async function formatWeeklyReportMarkdown(report) {
...
async function saveWeeklyReport(report, markdown) {
    const reportsDir = (0, path_1.join)(process.cwd(), 'ops', 'reports');
    await (0, promises_1.mkdir)(reportsDir, { recursive: true });
    const reportPath = (0, path_1.join)(reportsDir, 'FOUNDERS_WEEKLY_REPORT.md');
    await (0, promises_1.writeFile)(reportPath, markdown, 'utf-8');
    // Also save JSON for programmatic access
    const jsonPath = (0, path_1.join)(reportsDir, 'FOUNDERS_WEEKLY_REPORT.json');
