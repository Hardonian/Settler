...
router.get("/", auth_1.authMiddleware, tenant_1.tenantMiddleware, async (req, res) => {
    try {
        ...
        const serializedResults = results.map((result) => (0, serializers_1.serializeReconResult)(result));
        return res.json({
...
router.get("/:resultId", auth_1.authMiddleware, tenant_1.tenantMiddleware, async (req, res) => {
    try {
        ...
        return res.json({ data: (0, serializers_1.serializeReconResult)(result) });
    }
    catch (error) {
