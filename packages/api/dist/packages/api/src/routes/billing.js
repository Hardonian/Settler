 ...
 */
router.post("/create-customer", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { data: billingAccountData, error: fetchError } = await client_1.supabase
        ...
        if (fetchError && fetchError.code !== "PGRST116") {
            (0, logger_1.logError)("Error fetching billing account", fetchError);
        ...
        if (!billingAccount) {
            const { data: newBillingAccount, error: createError } = await client_1.supabase
                .from("billing_accounts")
                .insert({
                user_id: userId,
                tenant_id: tenantId,
                ...
                .single();
            if (createError) {
                (0, logger_1.logError)("Error creating billing account", createError);
                return res.status(500).json({
                    error: "Internal Server Error",
            ...
            const { data: updatedAccount, error: updateError } = await client_1.supabase
                .from("billing_accounts")
                ...
                .single();
            if (updateError) {
                (0, logger_1.logError)("Error updating billing account", updateError);
                return res.status(500).json({
                    error: "Internal Server Error",
        ...
        (0, logger_1.logInfo)("Billing account created/retrieved", {
        ...
        (0, logger_1.logError)("Failed to create customer", error);
        return res.status(500).json({
            error: "Internal Server Error",
...
router.post("/subscribe", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { data: billingAccount, error: accountError } = await client_1.supabase
            .from("billing_accounts")
            .select("*")
        ...
        const { data: existingSubscription } = await client_1.supabase
            .from("subscriptions")
            .select("*")
        ...
        const { data: subscription, error: subError } = await client_1.supabase
            .from("subscriptions")
            .insert({
            billing_account_id: billingAccountId,
            ...
            .single();
        if (subError) {
            (0, logger_1.logError)("Error creating subscription", subError);
        ...
        (0, logger_1.logInfo)("Subscription created", {
        ...
        (0, logger_1.logError)("Failed to create subscription", error);
        return res.status(500).json({
            error: "Internal Server Error",
            message: "Failed to create subscription",
        });
 ...
 */
router.post("/addon/purchase", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const { data: existingPurchase } = await client_1.supabase
        ...
        const { data: purchase, error: purchaseError } = await client_1.supabase
            .from("add_on_purchases")
            .insert({
            ...
            .single();
        if (purchaseError) {
            (0, logger_1.logError)("Error creating add-on purchase", purchaseError);
        ...
        (0, logger_1.logInfo)("Add-on purchased", {
        ...
        (0, logger_1.logError)("Failed to purchase add-on", error);
        return res.status(500).json({
            error: "Internal Server Error",
 ...
 */
router.post("/usage/report", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { billing_account_id, event_type, quantity, integration_id, add_on_id, unit, metadata } = req.body;
        ...
        const { data: eventId, error: rpcError } = await client_1.supabase.rpc("log_usage_event", {
        ...
        if (rpcError) {
            (0, logger_1.logError)("Error logging usage event", rpcError);
            return res.status(500).json({
                error: "Internal Server Error",
        ...
        (0, logger_1.logError)("Failed to report usage", error);
        return res.status(500).json({
            error: "Internal Server Error",
 ...
 */
router.get("/invoice/estimate", auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const billingAccountId = req.query.billing_account_id;
        ...
        const { data: bill, error: computeError } = await client_1.supabase.rpc("compute_estimated_bill", {
            p_billing_account_id: billingAccountId,
            p_start_date: startDate || null,
            p_end_date: endDate || null,
        });
        if (computeError) {
            (0, logger_1.logError)("Error computing bill", computeError);
            return res.status(500).json({
                error: "Internal Server Error",
        ...
        (0, logger_1.logError)("Failed to estimate invoice", error);
        return res.status(500).json({
            error: "Internal Server Error",
...
router.post("/webhook", async (req, res) => {
    try {
        ...
        if (!webhookSecret) {
            (0, logger_1.logError)("Stripe webhook secret not configured", new Error("Missing STRIPE_WEBHOOK_SECRET"));
            return res.status(500).json({
                error: "Internal Server Error",
        ...
        // Use the global supabase client
        // Log webhook event
        await client_1.supabase.from("stripe_event_log").insert({
        ...
        switch (event.type) {
            ...
            case "customer.subscription.deleted": {
                ...
                await handleSubscriptionUpdate(client_1.supabase, subscription);
                break;
            }
            case "invoice.paid":
            case "invoice.payment_failed": {
                const invoice = event.data.object;
                await handleInvoiceEvent(client_1.supabase, invoice);
                break;
            }
            case "invoice.upcoming": {
                const invoice = event.data.object;
                await handleInvoiceUpcoming(client_1.supabase, invoice);
                break;
            }
            default:
                (0, logger_1.logInfo)("Unhandled Stripe webhook event", { type: event.type });
        }
        ...
        (0, logger_1.logError)("Failed to handle webhook", error);
        return res.status(500).json({
            error: "Internal Server Error",
...
supabase, subscription) {
    ...
    if (error) {
        (0, logger_1.logError)("Error updating subscription", error);
    }
}
// Helper function to handle invoice events
async function handleInvoiceEvent(_supabase, invoice) {
    // In a real implementation, you'd:
    // 1. Update invoice status in database
    // 2. Send email notifications
    // 3. Update billing account status if payment failed
    (0, logger_1.logInfo)("Invoice event processed", {
