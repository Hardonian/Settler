...
router.post("/:id/run", (0, authorization_1.requirePermission)(Permissions_1.Permission.JOBS_WRITE), (0, validation_1.validateRequest)(getJobSchema), async (req, res) => {
    ...
    try {
        ...
        const jobs = await (0, db_1.query)(`SELECT status, version FROM jobs WHERE id = $1 AND user_id = $2 AND tenant_id = $3`, [id, userId, tenantId]);
        ...
        const updated = await (0, db_1.query)(`UPDATE jobs
         ...
         RETURNING id`, [id, userId, tenantId, job.version]);
        ...
        await (0, db_1.query)(`INSERT INTO audit_logs (event, user_id, tenant_id, metadata)
         VALUES ($1, $2, $3, $4)`, ["job_executed", userId, tenantId, JSON.stringify({ jobId: id, executionId })]);
