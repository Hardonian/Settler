...
router.delete("/:id/data", (0, authorization_1.requirePermission)(Permissions_1.Permission.USERS_DELETE), (0, validation_1.validateRequest)(deleteUserDataSchema), async (req, res) => {
    try {
        ...
        if (id !== userId) {
            ...
            const users = await (0, db_1.query)("SELECT role FROM users WHERE id = $1 AND tenant_id = $2", [userId, tenantId]);
            if (users.length === 0 ||
                !users[0] ||
        ...
        // Verify password
        const targetUsers = await (0, db_1.query)("SELECT password_hash FROM users WHERE id = $1 AND tenant_id = $2", [id, tenantId]);
        ...
        await (0, db_1.transaction)(async (client) => {
           ...
           SET deleted_at = NOW(),
               ...
               email = $1,
               name = 'Deleted User'
           WHERE id = $2 AND tenant_id = $3`, [`deleted-${id}@settler.io`, id, tenantId]);
...
router.get("/:id/data-export", (0, authorization_1.requirePermission)(Permissions_1.Permission.USERS_READ), (0, validation_1.validateRequest)(exportUserDataSchema), async (req, res) => {
    try {
        ...
        const [users, jobs, reports, webhooks, apiKeys, auditLogs] = await Promise.all([
           ...
           FROM jobs WHERE user_id = $1 AND tenant_id = $2`, [userId, tenantId]),
            (0, db_1.query)(`SELECT r.id, r.job_id, r.summary, r.generated_at
           FROM reports r
           JOIN jobs j ON r.job_id = j.id
           WHERE j.user_id = $1 AND j.tenant_id = $2`, [userId, tenantId]),
            (0, db_1.query)(`SELECT id, url, events, status, created_at, updated_at
           FROM webhooks WHERE user_id = $1 AND tenant_id = $2`, [userId, tenantId]),
            (0, db_1.query)(`SELECT id, name, scopes, rate_limit, created_at, last_used_at
           FROM api_keys WHERE user_id = $1 AND tenant_id = $2`, [userId, tenantId]),
            (0, db_1.query)(`SELECT event, metadata, timestamp
           FROM audit_logs
           WHERE user_id = $1 AND tenant_id = $2
           ORDER BY timestamp DESC
           LIMIT 1000`, [userId, tenantId]),
        ]);
        const exportData = {
            user: users[0],
            jobs: jobs,
