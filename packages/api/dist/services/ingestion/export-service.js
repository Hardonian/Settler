 ...
 */
function generateSignedUrl(filePath, expiresInHours = 24) {
...
async function createExport(options) {
    ...
    try {
        await (0, db_1.query)(`INSERT INTO exports (
      ...
      RETURNING id`, [
        ...
        (0, logger_1.logInfo)("Created export", { exportId, type: options.type, format: options.format, traceId });
        return exportId;
    }
    catch (error) {
        (0, logger_1.logError)("Failed to create export", error, { options });
        throw error;
    }
}
/**
 ...
 */
async function exportUnmatchedToCSV(reconciliationRunId, tenantId) {
    ...
    const csvRows = [];
    // Header
    csvRows.push("Transaction ID,Amount,Currency,Date,Description,External ID,Category,Payment Method");
    // Data rows
 ...
 */
async function exportAllToCSV(ingestionId, tenantId) {
    ...
    ORDER BY date DESC`, [ingestionId, tenantId]);
    const csvRows = [];
    // Header
 ...
 */
async function exportReconciliationReportToCSV(reconciliationRunId, tenantId) {
    // Get run summary
    ...
    ORDER BY rm.confidence DESC`, [reconciliationRunId, tenantId]);
    const csvRows = [];
    // Summary section
...
async function exportToJSON(options) {
    ...
    if (options.format === "matched" && options.reconciliationRunId) {
      ...
      JOIN normalized_transactions st ON st.id = rm.source_transaction_id
      LEFT JOIN normalized_transactions tt ON tt.id = rm.target_transaction_id
      WHERE rm.run_id = $1 AND rm.tenant_id = $2 AND rm.target_transaction_id IS NOT NULL`, [options.reconciliationRunId, options.tenantId]);
        data = matches;
    }
...
async function generateExport(exportId) {
    // Get export record
    const exportResults = await (0, db_1.query)(`SELECT type, format, reconciliation_run_id, ingestion_id, tenant_id
    FROM exports WHERE id = $1`, [exportId]);
    if (exportResults.length === 0) {
        throw new Error(`Export ${exportId} not found`);
    }
    const exportRecord = exportResults[0];
    // Update status to processing
    await (0, db_1.query)(`UPDATE exports SET status = 'processing', updated_at = NOW() WHERE id = $1`, [exportId]);
    let content;
    let fileExtension;
    try {
        if (exportRecord.type === "csv") {
            ...
            if (exportRecord.format === "matched" && exportRecord.reconciliation_run_id) {
                content = await exportMatchedToCSV(exportRecord.reconciliation_run_id, exportRecord.tenant_id);
            }
            else if (exportRecord.format === "unmatched" && exportRecord.reconciliation_run_id) {
                content = await exportUnmatchedToCSV(exportRecord.reconciliation_run_id, exportRecord.tenant_id);
            }
                ...
                content = await exportReconciliationReportToCSV(exportRecord.reconciliation_run_id, exportRecord.tenant_id);
            }
            else {
        ...
        await (0, db_1.query)(`UPDATE exports SET
        status = 'completed',
        storage_location = $1,
        signed_url = $2,
      ...
      WHERE id = $6`, [
        ...
        (0, logger_1.logInfo)("Export generated", { exportId, filePath, fileSize });
        return {
            filePath,
            fileSize,
            signedUrl: url,
