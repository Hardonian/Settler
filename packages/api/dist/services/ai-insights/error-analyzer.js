    ...
    try {
      ...
      LIMIT 50`, []);
        ...
        for (const error of errors) {
            ...
            const relatedErrors = [];
            if (count > 5) {
                const related = await (0, db_1.query)(`SELECT DISTINCT error_message
           FROM error_logs
           WHERE created_at > NOW() - INTERVAL '${interval}'
             AND severity = 'error'
             AND error_message != $1
           ...
           LIMIT 5`, [message, pattern, message.split(" ")[0] || ""]);
        ...
        (0, logger_1.logInfo)("Failed to analyze error patterns", {
            timeWindow,
