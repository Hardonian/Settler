...
exports.detectFeatureDependencies = detectFeatureDependencies;
exports.clusterUsersByBehavior = clusterUsersByBehavior;
exports.detectIncompleteWorkflows = detectIncompleteWorkflows;
const db_1 = require("../../db");
const logger_1 = require("../../utils/logger");
/**
 * Detect feature dependencies (which features users use together)
 */
async function detectFeatureDependencies(days = 30) {
    try {
        // Extract feature usage from analytics events
        // Features are identified by event names like: feature.accessed, usage.metric, etc.
        const featureEvents = await (0, db_1.query)(`SELECT DISTINCT user_id, 
        CASE 
      ...
      ORDER BY user_id, feature`, []);
        ...
        (0, logger_1.logInfo)("Failed to detect feature dependencies", {
            days,
 ...
 */
async function clusterUsersByBehavior() {
    try {
        // Get user behavior metrics
        const userMetrics = await (0, db_1.query)(`SELECT 
        u.id as user_id,
      ...
      GROUP BY u.id`, []);
        ...
        (0, logger_1.logInfo)("Failed to cluster users by behavior", {
            error: error instanceof Error ? error.message : String(error),
        });
        return [];
    }
 ...
 */
async function detectIncompleteWorkflows(userId) {
    try {
        const incomplete = [];
        // Detect incomplete reconciliation workflows
        const incompleteReconciliations = await (0, db_1.query)(`SELECT 
        j.user_id,
      ...
      ${userId ? "" : "LIMIT 100"}`, userId ? [userId] : []);
        ...
        (0, logger_1.logInfo)("Failed to detect incomplete workflows", {
            userId,
            error: error instanceof Error ? error.message : String(error),
        });
        return [];
