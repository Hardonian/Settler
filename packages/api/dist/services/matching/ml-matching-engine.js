...
exports.mlMatchingEngine = exports.MLMatchingEngine = void 0;
...
class MLMatchingEngine {
    modelVersion = "1.0.0";
    trainingDataCache = new Map();
    /**
     * Predict match using ML model
     * Falls back to deterministic algorithm if model not trained
     */
    async predictMatch(sourceTransactionId, targetTransactionIds, tenantId, sourceAdapter, targetAdapter) {
        try {
        ...
        FROM normalized_transactions
        WHERE id IN (${placeholders}) AND tenant_id = $1`, [tenantId, ...targetTransactionIds]);
            const targets = targetResults;
            // Extract features for each candidate
            const candidates = targets.map((target) => {
                const amountDiff = Math.abs(source.amount - target.amount);
                const dateDiff = Math.abs((source.date.getTime() - target.date.getTime()) / (1000 * 60 * 60 * 24));
                const descriptionSimilarity = this.calculateDescriptionSimilarity(source.description, target.description);
            ...
            const historicalMatchRate = await this.getHistoricalMatchRate(sourceAdapter, targetAdapter, tenantId);
            ...
            let matchType = "fuzzy";
            if (bestCandidate.features.externalIdMatch &&
            ...
            (0, logger_1.logError)("ML matching prediction failed", error, {
                sourceTransactionId,
                tenantId,
            });
            return null;
    ...
    async trainModel(tenantId) {
        try {
            (0, logger_1.logInfo)("Training ML model", { tenantId });
        ...
        LIMIT 10000`, [tenantId]);
            if (matches.length < 100) {
                (0, logger_1.logInfo)("Insufficient training data", { tenantId, matchCount: matches.length });
                return;
            }
            // Extract features from historical matches
            const trainingData = matches.map((match) => {
                const descriptionSimilarity = this.calculateDescriptionSimilarity(match.source_description, match.target_description);
                return {
            ...
            (0, logger_1.logInfo)("ML model training completed", {
                tenantId,
                trainingSamples: trainingData.length,
            });
        }
        catch (error) {
            (0, logger_1.logError)("ML model training failed", error, { tenantId });
        }
    }
    /**
     * Get historical match rate from cross-customer intelligence
     * This is a proprietary feature that competitors cannot replicate
     */
    async getHistoricalMatchRate(sourceAdapter, targetAdapter, tenantId) {
        try {
            // Query cross-customer intelligence for historical match rates
            // This aggregates anonymized patterns across all customers
            const { crossCustomerIntelligence } = await Promise.resolve().then(() => __importStar(require("../network-effects/cross-customer-intelligence")));
            ...
            const tenantMatches = await (0, db_1.query)(`SELECT COUNT(*) as total_matches
        FROM reconciliation_matches rm
        JOIN normalized_transactions st ON st.id = rm.source_transaction_id
        ...
        AND rm.match_type != 'unmatched'`, [tenantId, sourceAdapter, targetAdapter]);
            const totalMatches = tenantMatches[0]?.total_matches || 0;
            // Get total reconciliation attempts
            const totalAttempts = await (0, db_1.query)(`SELECT COUNT(*) as total_attempts
        FROM reconciliation_matches rm
        JOIN normalized_transactions st ON st.id = rm.source_transaction_id
        JOIN normalized_transactions tt ON tt.id = rm.target_transaction_id
        JOIN ingestion_sources si ON si.id = st.source_id
        ...
        WHERE rm.tenant_id = $1
        AND si.connector_type = $2
        AND ti.connector_type = $3`, [tenantId, sourceAdapter, targetAdapter]);
            const attempts = totalAttempts[0]?.total_attempts || 1;
        ...
        for (let i = 1; i <= len1; i++) {
            ...
            for (let j = 1; j <= len2; j++) {
                if (str1[i - 1] === str2[j - 1]) {
                    ...
                    row[j] = Math.min((prevRow[j] ?? 0) + 1, (row[j - 1] ?? 0) + 1, (prevRow[j - 1] ?? 0) + 1);
                }
            }
        }
...
exports.MLMatchingEngine = MLMatchingEngine;
exports.mlMatchingEngine = new MLMatchingEngine();
//# sourceMappingURL=ml-matching-engine.js.map