...
exports.MultiAgentFallback = void 0;
...
class MultiAgentFallback {
    ...
    async executeWithFallback(task, primaryModel) {
        ...
        for (const model of models) {
            try {
                (0, logger_1.logInfo)('Attempting task with model', { taskId: task.id, model, type: task.type });
                const result = await this.executeTask(task, model);
                if (result.success) {
                    (0, logger_1.logInfo)('Task succeeded', { taskId: task.id, model });
                    return result;
                }
                ...
                (0, logger_1.logError)('Task execution failed', { taskId: task.id, model, error: lastError });
            }
     ...
     */
    async handleIngestionFailure(error, context) {
     ...
     */
    async handleMappingUncertainty(uncertainFields, data) {
        return this.executeWithFallback({
            id: `mapping-${Date.now()}`,
            type: 'mapping',
     ...
     */
    async handleSchemaDeviation(expected, actual) {
        return this.executeWithFallback({
            id: `schema-${Date.now()}`,
...
exports.MultiAgentFallback = MultiAgentFallback;
//# sourceMappingURL=multi-agent-fallback.js.map