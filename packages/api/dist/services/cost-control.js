...
exports.costControlService = exports.CostControlService = exports.COST_DRIVERS = void 0;
const client_1 = require("../infrastructure/supabase/client");
const logger_1 = require("../utils/logger");
// Cost drivers enumeration
exports.COST_DRIVERS = {
    // Compute costs
...
class CostControlService {
     ...
     */
    async checkCostLimit(tenantId, billingAccountId, costDriverId, quantity = 1) {
        try {
            ...
            if (!driverLimits) {
                (0, logger_1.logWarn)(`No limits configured for cost driver: ${costDriverId}`, { tenantId });
            ...
            if (quantity > driverLimits.burst) {
                return {
                    allowed: false,
                    reason: `Burst limit exceeded. Max ${driverLimits.burst} ${exports.COST_DRIVERS[costDriverId]?.unit || 'units'} per request`,
            ...
            (0, logger_1.logError)('Error checking cost limit', error);
     ...
     * Record cost usage
     */
    async recordCostUsage(tenantId, billingAccountId, costDriverId, quantity = 1) {
        try {
            ...
            if (existing) {
                ...
                await client_1.supabase
                    ...
                    quantity,
                    unit: exports.COST_DRIVERS[costDriverId]?.unit || 'units',
                    metadata: {
                        cost_driver_id: costDriverId,
                        estimated_cost: quantity * (exports.COST_DRIVERS[costDriverId]?.baseCostPerUnit || 0),
                    },
                });
            }
            ...
            (0, logger_1.logError)('Error recording cost usage', error);
     ...
     * Get tenant cost limits
     */
    async getTenantCostLimits(tenantId, billingAccountId) {
        try {
            // Get subscription plan
            const { data: subscription } = await client_1.supabase
                .from('subscriptions')
            ...
            const { data: usageEvents } = await client_1.supabase
                .from('usage_events')
            ...
            (0, logger_1.logError)('Error getting tenant cost limits', error);
            return null;
        }
    }
    /**
     ...
     */
    async invalidateCostLimitsCache(_tenantId) {
     ...
     */
    async getEstimatedCost(tenantId, billingAccountId, period = 'monthly') {
        try {
            const limits = await this.getTenantCostLimits(tenantId, billingAccountId);
            if (!limits)
                return 0;
            ...
            Object.entries(limits.currentUsage).forEach(([driverId, usage]) => {
                const driver = exports.COST_DRIVERS[driverId];
                if (driver) {
            ...
            (0, logger_1.logError)('Error calculating estimated cost', error);
            return 0;
        }
    }
    /**
     ...
     */
    async detectAbuse(tenantId, billingAccountId) {
        try {
            const limits = await this.getTenantCostLimits(tenantId, billingAccountId);
            if (!limits) {
            ...
            (0, logger_1.logError)('Error detecting abuse', error);
            return { isAbuse: false, actions: [] };
        }
    }
}
exports.CostControlService = CostControlService;
exports.costControlService = new CostControlService();
