...
def generate_pdf_content(
    ...
    elements.append(
        Paragraph(f"Generated: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}", meta_style)
    )
    ...
    if records:
        # Determine columns
        fieldnames = columns[:10] if columns else list(records[0].keys())[:10]
...
def handle_export_pdf(job: Job) -> JobResult:
    ...
    try:
        ...
        logger.error(
            "PDF export failed", exc_info=True, tenant_id=tenant_id, entity_type=entity_type
        )
        return JobResult(
            success=False,
            error=str(e),
