...
from typing import (
    Any, Callable, Dict, Generic, List, Optional, Protocol, Set, Type, TypeVar, Union,
    cast, get_type_hints
)
...
class HookManager:
    ...
    def unsubscribe(self, hook_name: str, callback: Callable):
        ...
        if hook_name in self._subscribers:
            self._subscribers[hook_name] = [
                (p, cb) for p, cb in self._subscribers[hook_name]
                if cb != callback
            ]
    ...
    def execute(self, hook_name: str, *args, **kwargs) -> Any:
        ...
        for priority, callback in self._subscribers[hook_name]:
            try:
...
class PluginLoader:
    ...
    def load_from_entry_point(self, entry_point: str) -> Optional[Plugin]:
    ...
    def _create_plugin_from_module(self, module: Any) -> Optional[Plugin]:
    ...
    def _register_hooks_from_instance(self, plugin: Plugin):
        ...
        for name in dir(plugin.instance):
            ...
            if callable(method) and hasattr(method, '_is_hook'):
                hook_name = getattr(method, '_hook_name')
