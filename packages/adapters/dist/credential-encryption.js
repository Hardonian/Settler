...
async function decryptCredentials(encryptedCredentials, supabaseUrl, supabaseServiceKey) {
    ...
    const insecureAllowed = process.env.ALLOW_INSECURE_CREDENTIAL_FALLBACK === "true" &&
    ...
    if (process.env.ALLOW_INSECURE_CREDENTIAL_FALLBACK === "true" && !insecureAllowed) {
        console.error("[credential-encryption] ALLOW_INSECURE_CREDENTIAL_FALLBACK is set but rejected: requires NODE_ENV=development|test and ALLOW_INSECURE_CREDENTIAL_FALLBACK_IN_DEV=true (production must never use plaintext credential fallback)");
    }
    if (insecureAllowed) {
        console.warn("[credential-encryption] INSECURE legacy base64 credential fallback is active (dev/test only)");
        try {
