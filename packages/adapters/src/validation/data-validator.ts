...
export class DataValidator {
  ...
  validatePayout(payout: NormalizedPayout): ValidationResult {
    ...
    if (!payout.status) {
      ...
      !['pending', 'processing', 'completed', 'failed', 'cancelled'].includes(payout.status)
    ) {
  ...
  validateInvoice(invoice: NormalizedInvoice): ValidationResult {
    ...
    if (invoice.lineItems) {
      ...
      if (Math.abs(lineItemsTotal - invoice.amountCents) > tolerance) {
        warnings.push(
          `Line items total (${lineItemsTotal}) doesn't match invoice amount (${invoice.amountCents})`
        );
      }
    }
  ...
  validateAll(data: {
    ...
    warnings: string[];
    counts: {
      transactions: { valid: number; invalid: number };
      accounts: { valid: number; invalid: number };
      balances: { valid: number; invalid: number };
      payouts: { valid: number; invalid: number };
      invoices: { valid: number; invalid: number };
      subscriptions: { valid: number; invalid: number };
      taxEstimates: { valid: number; invalid: number };
    };
  } {
