...
export class TrueLayerDriver implements ConnectorDriver {
  readonly metadata: ConnectorMetadata = {
    ...
    authType: "oauth2",
    description:
      "Connect your European/UK bank accounts via TrueLayer for automatic transaction and balance sync",
    icon: "🏦",
  ...
  async getAuthUrl(options: AuthUrlOptions): Promise<string> {
    ...
    return await Promise.resolve(`${apiUrl}/connect/v1/authorize?${params.toString()}`);
  }
