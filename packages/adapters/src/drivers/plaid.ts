...
export class PlaidDriver implements ConnectorDriver {
  ...
  async getAuthUrl(options: AuthUrlOptions): Promise<string> {
    ...
    if (!response.ok) {
      const error = (await response.json()) as PlaidErrorResponse;
  ...
  async handleCallback(
    ...
    const data = (await response.json()) as PlaidTokenExchangeResponse;
    return {
  ...
  async refreshToken(
    ...
    await Promise.resolve(); // Satisfy async requirement before throwing
    throw new ConnectorError(
  ...
  > {
    ...
    try {
      ...
      if (!accountsResponse.ok) {
        const error = (await accountsResponse.json()) as PlaidErrorResponse;
        throw new ConnectorError(
      ...
      for (const account of accountsData.accounts) {
        accounts.push({
          ...
          institutionName: accountsData.item.institution_id ?? undefined,
          institutionId: accountsData.item.institution_id ?? undefined,
          metadata: {
      ...
      if (!transactionsResponse.ok) {
        const error = (await transactionsResponse.json()) as PlaidErrorResponse;
        throw new ConnectorError(
      ...
      const transactionsData = (await transactionsResponse.json()) as PlaidTransactionsSyncResponse;
      ...
      for (const tx of transactionsData.added) {
        ...
        transactions.push({
          ...
          referenceId: tx.pending_transaction_id ?? undefined,
