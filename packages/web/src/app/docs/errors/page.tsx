...
export default function ErrorsPage() {
  return (
    <div className="prose prose-slate dark:prose-invert max-w-none">
      ...
      <p>
        This guide covers common API errors and how to resolve them. If you encounter an error not
        listed here, please <a href="/support">contact support</a>.
      </p>

      {commonErrors.map((error) => (
        <section key={error.code} className="my-8">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4">
            <h2 className="text-xl font-bold text-red-900 dark:text-red-200 mb-2">{error.code}</h2>
            <p className="text-red-800 dark:text-red-300 font-medium mb-1">{error.message}</p>
            <p className="text-sm text-red-700 dark:text-red-400">{error.description}</p>
          </div>
