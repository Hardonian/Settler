...
export default async function RunDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!run) {
    return (
      <div className="rounded border border-border bg-card p-4 text-foreground">Run not found.</div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h1 className="text-2xl font-semibold text-foreground">Run {run.id}</h1>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <section className="rounded border border-border bg-card p-4 text-sm">
        ...
        <section className="rounded border border-border bg-card p-4 text-sm">
          <h2 className="font-semibold text-foreground">Deterministic replay status</h2>
          {replay ? (
            <div className="mt-2 space-y-1 text-muted-foreground">
      ...
      <section className="rounded border border-border bg-card p-4 text-sm">
        <h2 className="font-semibold text-foreground">Evidence and lineage context</h2>
        {evidence ? (
          <div className="mt-2 space-y-1 text-muted-foreground">
          ...
          <p className="mt-2 text-muted-foreground">Evidence endpoint unavailable for this run.</p>
        )}
        <div className="mt-4 flex flex-wrap gap-4">
          <Link href={`/app/proofs/${run.id}`} className="font-medium text-primary hover:underline">
          ...
          <Link href="/app/alerts" className="font-medium text-primary hover:underline">
            Open Live Alerts →
          </Link>
        </div>
      </section>
