...
const getActionBadge = (action: string) => {
  switch (action.toLowerCase()) {
    case 'create': return <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20">CREATE</Badge>;
    case 'update': return <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20">UPDATE</Badge>;
    case 'delete': return <Badge className="bg-destructive/10 text-destructive border-destructive/20">DELETE</Badge>;
    ...
    <div className="space-y-12 pb-12">
      <div>
        ...
        <p className="mt-4 max-w-2xl text-base text-muted-foreground leading-relaxed">
          The immutable ledger of all workspace activities. 
          Audit trails track mutations across policies, data connections, 
          and reconciliation runs to maintain a verifiable chain of custody.
        </p>
      </div>
      ...
      <section className="space-y-6">
         <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="font-mono text-[10px]">{logs.length} ENTRIES</Badge>
         </div>

         <div className="space-y-3">
            {logs.map((log) => (
               <Card key={log.id} className="group overflow-hidden border-border/40 shadow-none hover:bg-muted/10 transition-all border-l-2 border-l-transparent hover:border-l-primary/40">
                     ...
                     <div className="flex items-center justify-between gap-6">
                        <div className="flex items-center gap-4 min-w-0">
                           <div className="h-10 w-10 rounded-lg bg-muted text-muted-foreground flex items-center justify-center shrink-0">
                              {log.actor === 'system' ? <Database className="h-5 w-5" /> : <User className="h-5 w-5" />}
                           </div>
                        ...
                        <div className="flex items-center gap-6 shrink-0 text-right">
                           ...
                           <button className="rounded-lg p-2 text-muted-foreground hover:bg-primary/5 hover:text-primary transition-all">
