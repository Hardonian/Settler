...
import { ChevronLeft, ChevronRight, Copy, ExternalLink, Filter, Search, TrendingUp, X } from "lucide-react";
...
export function WorkspaceInvestigationClient() {
  const { investigation, filters, setFilters, isLoading, error, refresh } = useWorkspaceInvestigation();
  ...
  if (!investigation) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" asChild href="/workspace">
          Go to Workspace
        </Button>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <Button variant="outline" size="icon" asChild href="/workspace">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-muted rounded p-4">
              ...
              <p className="text-2xl font-bold text-foreground">{investigation.totalTransactions}</p>
            </div>
            <div className="bg-muted rounded p-4">
              <p className="text-xs text-muted-foreground font-medium">Matched</p>
              <p className="text-2xl font-bold text-foreground">{investigation.matchedTransactions}</p>
            </div>
            <div className="bg-muted rounded p-4">
              <p className="text-xs text-muted-foreground font-medium">Mismatched</p>
              <p className="text-2xl font-bold text-foreground">{investigation.mismatchedTransactions}</p>
            </div>
            <div className="bg-muted rounded p-4">
              <p className="text-xs text-muted-foreground font-medium">Requires Review</p>
              <p className="text-2xl font-bold text-foreground">{investigation.requiresReviewTransactions}</p>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-foreground">Matching Rules Applied</h3>
                      <p className="text-xs text-muted-foreground">{investigation.matchingRulesCount} rules</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-foreground">Auto-matched Transactions</h3>
                      <p className="text-xs text-muted-foreground">{investigation.autoMatchedCount} transactions</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-foreground">Manual Review Required</h3>
                      <p className="text-xs text-muted-foreground">{investigation.manualReviewCount} transactions</p>
                    </div>
                  </div>
                </div>
              </CardContent>
