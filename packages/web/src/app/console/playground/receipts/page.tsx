    ...
    try {
      ...
      if (res.ok) {
        ...
        setResult({
          ...
          currency: "USD",
          items: [
            { desc: "Latte Grande", amount: 5.5 },
            { desc: "Bagel", amount: 4.0 },
            { desc: "Croissant", amount: 5.0 },
          ],
        });
      }
    } catch (err) {
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400">
            Test the OCR and extraction engine with real receipts.
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800">
                ...
                <Tabs
                  ...
                  onValueChange={(v) => setSelectedLanguage(v as "node" | "python" | "curl")}
                >
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {result.items?.map((item: ReceiptItem, i: number) => (
                      <div
                        key={i}
                        className="flex justify-between items-center p-2 bg-slate-50 dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-800"
                      >
                        <span className="text-sm text-slate-700 dark:text-slate-300">
                          {item.desc || item.name}
                        </span>
                        <span className="text-sm font-medium text-slate-900 dark:text-white">
                          {item.amount?.toFixed(2) || item.lineTotal?.toFixed(2)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
                  <div className="relative group">
                    ...
                    <CopyButton
                      ...
                      className="absolute top-6 right-2 opacity-0 group-hover:opacity-100 transition-opacity"
                      size="sm"
                    />
                  </div>
                </div>
