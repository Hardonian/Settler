        ...
        <Card>
          ...
          <CardContent>
            {experiments && experiments.length > 0 ? (
              ...
              <div className="text-center py-12 text-muted-foreground">No experiments running.</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="p-8">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          ...
          <p className="text-sm text-red-800 dark:text-red-300">
            We encountered an error while loading experiments. Please try again or contact support
            if the problem persists.
          </p>
          {process.env.NODE_ENV === "development" && (
            <p className="text-xs font-mono text-red-600 dark:text-red-400 mt-2">{errorMessage}</p>
          )}
        </div>
      </div>
    );
