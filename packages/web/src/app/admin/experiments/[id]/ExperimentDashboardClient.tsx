...
export default function ExperimentDashboardClient({ experiment }: ExperimentDashboardClientProps) {
  ...
  const toggleStatus = async () => {
    ...
    setStatus(newStatus);
    // Auto save on toggle - call async function directly, then use startTransition for UI
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-3">
            ...
            <Badge
              ...
              className={status === "running" ? "bg-green-100 text-green-700" : ""}
            >
              {status}
            </Badge>
          </h1>
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={toggleStatus} disabled={isPending}>
            {status === "running" ? (
              <>
                <Pause size={16} className="mr-2" /> Pause Experiment
              </>
            ) : (
              <>
                <Play size={16} className="mr-2" /> Start Experiment
              </>
            )}
          </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t text-sm text-muted-foreground flex justify-between">
              ...
              <span
                className={
                  Object.values(split).reduce((a: number, b: any) => a + b, 0) !== 100
