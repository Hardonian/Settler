        ...
        <div className="flex items-center gap-3 p-4 border border-border bg-card rounded-xl">
          <div className="flex flex-col">
            ...
            <p className="text-xs text-muted">
              Resolved as:{" "}
              <span
                className={`uppercase font-mono px-2 py-0.5 rounded-md text-[10px] space-x-2 ${statusColor}`}
              >
                {displayStatus}
              </span>
              {" | "} By {reviewState.reviewedBy || "System Operator"} on{" "}
              {new Date(reviewState.createdAt).toLocaleString()}
            </p>
            {reviewState.notes && (
              <p className="text-xs mt-2 italic border-l-2 border-border pl-2 text-muted">
                {reviewState.notes}
              </p>
            )}
          </div>
            ...
            <dl className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm">
              ...
              <div className="col-span-1">
                ...
                <dd className="text-foreground mt-1">
                  {new Date(sourceTx.timestamp).toLocaleString()}
                </dd>
              </div>
              <div className="col-span-1">
                <dt className="text-muted font-medium">Amount</dt>
                <dd className="font-mono font-medium text-xl mt-1 text-teal-500">
              ...
              <div className="col-span-1">
                ...
                <dd className="text-foreground mt-1">
                  {sourceTx.description || sourceTx.vendor || "N/A"}
                </dd>
              </div>
            </dl>
          </CardContent>
            ...
            {matchResult?.targetTransactionId ? (
              <div className="text-sm p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-md">
                <p className="font-semibold text-emerald-600 mb-2">
                  Algorithm detected a valid counterpart (Type: {matchResult.matchType})
                </p>
                <div className="font-mono text-emerald-800 break-all">
                  {matchResult.targetTransactionId}
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center p-6 text-center border-dashed border-2 border-border/50 rounded-xl bg-card">
                ...
                <p className="text-sm text-muted max-w-sm mx-auto mt-1">
                  The deterministic model rejected possible matches due to threshold violations or
                  lacking parity.
                </p>
              </div>
            )}
          ...
          <div className="pr-4">
            <div className="mb-4">
              <h4 className="text-sm font-semibold mb-2 text-foreground">
                Possible Ledger Matches
              </h4>
              {potentialTargets.length === 0 && (
                <div className="text-sm text-muted py-2">
                  No alternative items near the amount variance.
                </div>
              )}
            ...
            <div className="space-y-3 mt-6">
              <input
                ...
                disabled={
                  ...
                  isPending ||
                  !selectedTarget ||
                  selectedTarget === matchResult?.targetTransactionId
                }
              />
              <Button
                variant="outline"
          ...
          <div className="pl-4">
            <div className="bg-rose-500/5 border border-rose-500/20 p-5 rounded-xl">
              ...
              <p className="text-xs text-rose-600/80 mb-4">
                If this item is an exception that will never reconcile natively, force resolution
                without a ledger destination.
              </p>
