  ...
  withUniversalBillingGate(
    async function POST(request: NextRequest) {
      try {
        ...
        const existingResult = await ((supabase.from("user_milestones") as any)
          .select("id")
          .eq("user_id", user.id)
        ...
        // Create milestone
        const milestoneResult = await ((supabase.from("user_milestones") as any)
          .insert({
            user_id: user.id,
  ...
  withUniversalBillingGate(
    async function GET(request: NextRequest) {
      try {
        ...
        const finalQuery = milestoneType ? query.eq("milestone_type", milestoneType) : query;

        const { data, error } = await (finalQuery as Promise<{
          data: Array<{ milestone_type: string }> | null;
          error: { message?: string } | null;
        }>);
