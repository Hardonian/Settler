...
export const GET = withSecurity(
  withUniversalBillingGate(
    async function GET(request: NextRequest, { params }: { params: { workspaceId: string } }) {
      ...
      try {
        ...
        const { data: membership } = await (supabase.from("tenant_users") as any)
          .select("role")
        ...
        const { data: progress, error } = await (supabase.from("tenant_onboarding_progress") as any)
          .select("*")
          .eq("tenant_id", params.workspaceId)
          .eq("user_id", user.id)
          .single();
        ...
        if (!progress) {
          const { data: newProgress, error: createError } = await (
            supabase.from("tenant_onboarding_progress") as any
          )
            .insert({
...
export const POST = withSecurity(
  withUniversalBillingGate(
    async function POST(request: NextRequest, { params }: { params: { workspaceId: string } }) {
      const traceId = await getTraceId(request);
...
const ONBOARDING_STEPS = [
  ...
  },
  {
    ...
    description: "Inspect reconciliation results from completed tenant-scoped runs",
