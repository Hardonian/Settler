...
export const GET = withSecurity(
  publicRoute(async function GET() {
    ...
    try {
      ...
      try {
        const { error: queryError } = await supabase.from("api_keys").select("id").limit(1);
        ...
        if (queryError) {
          ...
          if (queryError.code === "42P01" || queryError.message.includes("does not exist")) {
            ...
            health.checks.supabase.error =
              "api_keys table does not exist (migration may be needed)";
            health.status = "degraded";
          } else {
            health.checks.supabase.status = "error";
        ...
        health.checks.supabase.status = "error";
        health.checks.supabase.error =
          queryErr instanceof Error ? queryErr.message : "Unknown query error";
        health.status = "degraded";
      }
      ...
      try {
        ...
        health.checks.auth.error =
          authErr instanceof Error ? authErr.message : "Unknown auth error";
        health.status = "degraded";
      }
