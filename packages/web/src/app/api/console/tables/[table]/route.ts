...
// Raw database table viewer. Subscription tier alone is insufficient —
// any cross-tenant data would be exposed to regular subscribers.
 ...
 * Restricted to super admins to prevent cross-tenant data exposure.
 */
...
export const GET = withSecurity(
  withUniversalBillingGate(
    async function GET(request: NextRequest) {
      try {
        // Super admin gate — raw table access exposes cross-tenant data.
        const adminOk = await isSuperAdmin();
        if (!adminOk) {
          return NextResponse.json(
            { error: "Forbidden", message: "Super admin access required for raw table viewer" },
            { status: 403 }
          );
        }
        ...
        const filters = searchParams.get("filters");
        if (filters) {
          const filterObj = safeJsonParseWithDefault<Record<string, unknown>>(
            filters,
            {},
            "table filters fallback"
          );
          ...
          return NextResponse.json(
            {
              ...
              actionable:
                "Please verify your connection and try again. If the problem persists, contact support at support@settler.dev.",
              data: [],
              count: 0,
              limit,
              offset,
        ...
        return NextResponse.json(
          {
            ...
            message:
              error instanceof Error
                ? error.message
                : "An unexpected error occurred while fetching table data.",
            actionable:
              "Please verify your connection and try again. If the problem persists, contact support at support@settler.dev.",
            data: [],
            count: 0,
...
export const POST = withSecurity(
  withUniversalBillingGate(
    async function POST(request: NextRequest) {
      try {
        // Super admin gate — raw table writes bypass application-level validation.
        const adminOk = await isSuperAdmin();
        if (!adminOk) {
          return NextResponse.json(
            { error: "Forbidden", message: "Super admin access required for raw table writes" },
            { status: 403 }
          );
        }
          ...
          return NextResponse.json(
            {
              ...
              actionable:
                "Please verify your data format and try again. If the problem persists, contact support at support@settler.dev.",
              retryable: true,
            },
            { status: 200 }
          );
  ...
  withUniversalBillingGate(
    async function DELETE(request: NextRequest) {
      try {
        ...
        if (!adminOk) {
          return NextResponse.json(
            { error: "Forbidden", message: "Super admin access required for raw table deletes" },
            { status: 403 }
          );
        }
          ...
          return NextResponse.json(
            {
              ...
              actionable:
                "Please verify the record ID and try again. If the problem persists, contact support at support@settler.dev.",
              success: false,
              retryable: true,
            },
            { status: 200 }
