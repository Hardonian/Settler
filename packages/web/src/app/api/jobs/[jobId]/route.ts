    ...
    async function GET(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
      ...
      try {
        ...
        const response: JobDetailResponse = {
          ...
          results: job.results.map((result: (typeof job.results)[number]) => ({
            id: result.id,
            ...
            totalAmountMatched: result.totalAmountMatched
              ? Number(result.totalAmountMatched)
              : null,
            totalAmountUnmatched: result.totalAmountUnmatched
              ? Number(result.totalAmountUnmatched)
              : null,
          ...
          latestResult: latestResult
            ? {
                ...
                totalAmountSource: latestResult.totalAmountSource
                  ? Number(latestResult.totalAmountSource)
                  : null,
                totalAmountTarget: latestResult.totalAmountTarget
                  ? Number(latestResult.totalAmountTarget)
                  : null,
                totalAmountMatched: latestResult.totalAmountMatched
                  ? Number(latestResult.totalAmountMatched)
                  : null,
                totalAmountUnmatched: latestResult.totalAmountUnmatched
                  ? Number(latestResult.totalAmountUnmatched)
                  : null,
                currency: latestResult.currency,
                confidenceAvg: latestResult.confidenceAvg
                  ? Number(latestResult.confidenceAvg)
                  : null,
                confidenceMin: latestResult.confidenceMin
                  ? Number(latestResult.confidenceMin)
                  : null,
                confidenceMax: latestResult.confidenceMax
                  ? Number(latestResult.confidenceMax)
                  : null,
