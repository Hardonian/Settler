...
export const POST = withSecurity(
  ...
  try {
    ...
    if (idempotencyCheck.isDuplicate && idempotencyCheck.existingResponse) {
      logger.info('Returning cached sync result (idempotency)', { correlationId, idempotencyKey });
      return NextResponse.json({
        success: true,
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error('Sync operation timed out after 4 minutes')), 4 * 60 * 1000);
    });
    ...
    try {
      ...
      const { tenantId, since, until, accountId } = body as { tenantId?: string; since?: string; until?: string; accountId?: string };
      if (tenantId) {
        const supabase = await createClient();
