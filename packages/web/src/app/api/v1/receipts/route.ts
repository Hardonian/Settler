...
} from "@/domain/receipts/validation";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const ocrResult = await ocrProvider.extractText(imageData, mimeType || "image/jpeg");
      ...
      logger.info("Receipt parsed from OCR", {
        correlationId,
        itemCount: parseResult.receipt.items.length,
      });

      // Validate and sanitize receipt data
      const sanitizedData = sanitizeReceiptData(
        parseResult.receipt as unknown as Record<string, unknown>
      );
      ...
      const totalsValidation = validateReceiptTotals(
        sanitizedData as {
          subtotal?: number | null;
          tax?: number | null;
          total?: number | null;
      ...
      if (auth && auth.userId && auth.billingAccountId) {
        await logReceiptParsed(auth.userId, auth.billingAccountId, receipt.id, {
          itemCount: parseResult.receipt.items.length,
          total: receipt.total,
      ...
      const receiptWithItems = (await withRetry(() =>
        prisma.receipt.findUnique({
          where: { id: receipt.id },
          include: { items: true },
        })
      )) as { items: ReceiptItem[] } | null;
