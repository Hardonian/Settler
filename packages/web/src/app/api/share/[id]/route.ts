  ...
  withUniversalBillingGate(
    async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
      try {
        ...
        const artifactResult = await ((supabase.from("shareable_artifacts") as any)
          .select("*")
          .eq("id", id)
          .single() as Promise<{
        ...
        if (typedArtifact.artifact_type === "reconciliation_report") {
          const reportResult = await ((supabase.from("reconciliation_jobs") as any)
            .select("*")
        ...
        if (typedArtifact.artifact_type === "receipt") {
          const receiptResult = await ((supabase.from("receipts") as any)
            .select("*")
            .eq("id", typedArtifact.artifact_id)
            .single() as Promise<{
            data: Record<string, unknown> | null;
