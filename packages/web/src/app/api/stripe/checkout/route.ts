...
function isValidPlanCode(planCode: unknown): planCode is PlanCode {
  return (
    typeof planCode === "string" && ["starter", "growth", "scale", "enterprise"].includes(planCode)
  );
}
    ...
    try {
      ...
      if (planConfig && planConfig.code === "starter") {
        return NextResponse.json(
          {
            error:
              "Cannot upgrade to starter plan. Please use customer portal to cancel subscription.",
          },
          { status: 400 }
        );
      }
      ...
      try {
        ...
        await emitLifecycleEventSafe(LifecycleEventType.BILLING_CHECKOUT_STARTED as string, {
          userId: user.id,
          tenantId: tenant?.id,
