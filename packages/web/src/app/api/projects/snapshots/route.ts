...
export const POST = withUniversalBillingGate(
  async function POST(request: NextRequest) {
    try {
      ...
      if (projectType === "job") {
        const jobResult = await ((supabase.from("reconciliation_jobs") as any)
          .select("*")
          .eq("id", projectId)
          .eq("user_id", user.id)
          .single() as Promise<{
        ...
        const integrationResult = await ((supabase.from("integration_credentials") as any)
          .select("*")
          .eq("id", projectId)
          .eq("user_id", user.id)
          .single() as Promise<{
      ...
      // Create snapshot
      const snapshotResult = await ((supabase.from("project_snapshots") as any)
        .insert({
          user_id: user.id,
