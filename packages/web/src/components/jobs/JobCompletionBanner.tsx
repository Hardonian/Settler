...
export function JobCompletionBanner({
  ...
  return (
    <Card
      className={`border-2 shadow-lg mb-6 ${
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-foreground dark:text-muted-foreground mb-4">
              <strong>{jobName}</strong> finished processing at{" "}
              {new Date(completedAt).toLocaleTimeString()}.
              {isHighAccuracy && !hasUnmatched ? (
                <>
                  {" "}
                  All {matchedCount.toLocaleString()} transactions matched successfully with{" "}
                  {accuracy}% accuracy.
                </>
              ) : hasUnmatched ? (
                <>
                  {" "}
                  {matchedCount.toLocaleString()} transactions matched ({accuracy}% accuracy).{" "}
                  {unmatchedCount} transaction{unmatchedCount !== 1 ? "s" : ""} need
                  {unmatchedCount === 1 ? "s" : ""} your review.
                </>
              ) : (
                <>
                  {" "}
            ...
            <div className="flex flex-wrap gap-3">
              <Button
                asChild
                size="sm"
                variant={isHighAccuracy && !hasUnmatched ? "default" : "outline"}
              >
                <Link href={`/dashboard/jobs/${jobId}`}>View Full Report</Link>
              </Button>
              {hasUnmatched && (
                <Button
                  asChild
                ...
                >
                  <Link href={`/dashboard/jobs/${jobId}#unmatched`}>Review Unmatched</Link>
                </Button>
              )}
