...
export function EnvErrorPanel({ missingVars, isFirstRun = false }: EnvErrorPanelProps) {
  ...
  const hasSupabaseVars = missingVars.some(v => 
    v.includes('SUPABASE') || v.includes('DATABASE')
  );
        ...
        <CardContent className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ol className="text-xs text-blue-700 dark:text-blue-300 mt-2 space-y-1 list-decimal list-inside">
                    <li>Copy <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">.env.example</code> to <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">.env.local</code></li>
                    <li>Add your Supabase credentials</li>
                    <li>Run <code className="bg-blue-100 dark:bg-blue-900 px-1 rounded">pnpm db:push</code> to set up the database</li>
                  </ol>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <ul className="list-disc list-inside text-sm text-slate-600 dark:text-slate-400 space-y-1">
              {missingVars.map((key) => (
                <li key={key} className="font-mono text-xs bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded inline-block mr-2">
                  {key}
                </li>
              ))}
            </ul>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-4">
            <div className="flex items-start gap-2">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">
                ...
                <code className="bg-slate-200 dark:bg-slate-800 px-1 rounded">.env.example</code>{' '}
                to <code className="bg-slate-200 dark:bg-slate-800 px-1 rounded">.env.local</code>{' '}
          ...
          {isFirstRunScenario && (
            <p className="text-xs text-center text-muted-foreground pt-2">
              Need help? Check the <Link href="/docs" className="underline hover:text-foreground">documentation</Link> or <Link href="/support" className="underline hover:text-foreground">contact support</Link>.
            </p>
          )}
        </CardContent>
      </Card>
