...
export function EnhancedPlayground() {
  ...
  useEffect(() => {
    ...
    fetch("/api/console/api-keys")
      ...
      .then((data) => {
        ...
        if (active?.keyPrefix) {
          setApiKeyHint(
            `Active key prefix: ${active.keyPrefix}… — paste the full secret to call the API.`
          );
        }
      })
      .catch((err: unknown) => {
  ...
  const apiCall: ApiCall = {
    ...
    body: parsedRequestBody.ok
      ? (parsedRequestBody.data as Record<string, unknown> | undefined)
      : undefined,
  };
  ...
  return (
    <ConsoleErrorBoundary>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2 leading-[1.4]">API Playground</h1>
        ...
        {/* Code Generator */}
        {apiKey && <CodeGenerator apiCall={apiCall} apiKey={apiKey} />}
      </div>
    </ConsoleErrorBoundary>
  );
}
