...
function LoadingSkeleton({ className, lines = 3 }: { className?: string; lines?: number }) {
  return (
...
function LoadingState({ variant = "spinner", size = "md", message, className }: LoadingStateProps) {
  const sizeClasses = {
    sm: "p-4",
    md: "p-8",
    lg: "p-12",
  ...
  return (
    ...
    >
      ...
      {message && <p className="text-sm text-muted-foreground text-center max-w-md">{message}</p>}
    </div>
  );
}
