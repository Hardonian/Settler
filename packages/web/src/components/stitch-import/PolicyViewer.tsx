    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
           <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20">{policies.length} TOTAL</Badge>
      ...
      <div className="space-y-3">
        {policies.length > 0 ? (
          policies.map((policy) => (
            <Card key={policy.id} className="group overflow-hidden border-border/40 shadow-none hover:bg-muted/10 transition-all border-l-2 border-l-transparent hover:border-l-primary/40">
              <CardContent className="p-4 sm:p-5">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className={cn(
                      "p-2 rounded-lg shrink-0",
                      policy.status === "active" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                    )}>
                      {policy.status === "active" ? <ShieldCheck className="h-5 w-5" /> : <Scale className="h-5 w-5" />}
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="text-sm font-bold text-foreground truncate">{policy.name}</h4>
                        <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-widest px-1.5 h-4 border-muted-foreground/20">
                          {policy.version}
                        </Badge>
                      </div>
          ...
          <div className="py-20 text-center border-2 border-dashed border-border/40 rounded-2xl bg-muted/20">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mx-auto mt-1 leading-relaxed">
              Your workspace posture is currently undefined. Connect a reconciliation policy to see live enforcement.
            </p>
      ...
      <Card className="bg-primary/5 border-primary/20 shadow-none mt-8">
        <CardContent className="p-4 flex gap-3 items-start">
           ...
           <div className="space-y-1">
             <h4 className="text-xs font-bold text-primary uppercase tracking-widest">Policy History Audit</h4>
             <p className="text-xs text-muted-foreground leading-relaxed">
               All contract version rotations and policy overrides are recorded in the immutable trust log for post-mortem evidence.
             </p>
