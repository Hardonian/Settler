    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Reconciliation Results</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Review the outcomes of your reconciliation pipelines.</p>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" className="rounded-full">
              <Share className="w-5 h-5 text-slate-500" />
            </Button>
            <Button variant="outline" size="icon" className="rounded-full bg-primary/10 border-primary/20 text-primary-600">
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
          <Button variant="primary" className="rounded-full px-4 h-10 shadow-lg shadow-primary-600/20 gap-2">
            All Status <ChevronDown className="w-4 h-4" />
          </Button>
          <Button variant="outline" className="rounded-full px-4 h-10 gap-2 border-slate-200 text-slate-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4 border-emerald-500/20 bg-emerald-50/10 relative overflow-hidden group">
          ...
          <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Matched</span>
        ...
        <Card className="p-4 border-rose-500/20 bg-rose-50/10 relative overflow-hidden group">
          ...
          <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Mismatch</span>
      ...
      <div className="space-y-4">
        <h2 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Recent Transactions</h2>
          ...
          {results.map((res) => (
            <Card
              ...
              className={cn(
                ...
                res.status === "Mismatched" ? "border-l-rose-500" :
                res.status === "Needs Review" ? "border-l-amber-500" : "border-l-emerald-500"
              )}
              onClick={() => setSelectedResult(res)}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  {res.status === "Mismatched" ? <AlertCircle className="w-4 h-4 text-rose-500" /> :
                   res.status === "Needs Review" ? <AlertTriangle className="w-4 h-4 text-amber-500" /> : <CheckCircle2 className="w-4 h-4 text-emerald-500" />}
                  <Badge variant={res.status === "Matched" ? "success" : res.status === "Mismatched" ? "destructive" : "warning"}>
                    {res.status}
                  </Badge>
                </div>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <div className="text-sm font-mono text-slate-500 mb-1">Trace: <span className="text-slate-900 dark:text-white font-bold">{res.traceId}</span></div>
                  <div className="text-sm font-bold text-slate-900 dark:text-white">Tx Ref: {res.ref}</div>
                  <div className="text-xs font-semibold text-slate-500 mt-1">Pipeline: {res.pipeline}</div>
                </div>
                <Button variant="ghost" size="sm" className="text-primary-600 font-bold gap-1 group">
            ...
            <div className="space-y-8 py-6">
              <SheetHeader>
                <div>
                   ...
                   <div className="flex items-center gap-2 mt-2">
                     {selectedResult.status === "Mismatched" ? <AlertCircle className="w-4 h-4 text-rose-500" /> : <AlertTriangle className="w-4 h-4 text-amber-500" />}
                     <span className={cn("text-sm font-bold", selectedResult.status === "Mismatched" ? "text-rose-500" : "text-amber-500")}>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-xs font-bold uppercase text-slate-500 tracking-wider">Comparison Matrix</h3>
                  <Button variant="link" size="sm" className="h-auto p-0 font-bold text-primary-600">View Raw JSON</Button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <Card className="p-4 bg-rose-50/30 dark:bg-rose-950/20 border-rose-500/30 space-y-4 relative">
                      ...
                      <div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Amount</span>
                        <div className="font-mono text-sm font-black text-rose-600 mt-1">99.00 USD</div>
                      </div>
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-bold uppercase text-slate-500 tracking-wider">Logic Trace</h3>
                  <div className="space-y-3">
                    <div className="flex items-center gap-4 p-4 rounded-xl border border-emerald-500/20 bg-emerald-50/5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium">Both sources match ISO-4217 code: USD</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-4 rounded-xl border border-rose-500/20 bg-rose-50/5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-rose-500 font-medium font-mono">Mismatch detected: -1.00 USD (-1%)</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-8 flex gap-3">
                  <Button variant="outline" className="flex-1 h-12 font-bold" onClick={() => handleAction("ignore_mismatch")}>
                    Ignore Mismatch
                  </Button>
                  <Button className="flex-1 h-12 bg-primary-600 hover:bg-primary-700 text-white font-bold shadow-lg shadow-primary-600/20" onClick={() => handleAction("open_review")}>
