  ...
  test("returns productized proofpack artifact with explicit completeness and unavailable delta semantics", async () => {
    getReconciliationWorkbenchExceptionDetailMock.mockResolvedValue({
      ...
      exceptionIntelligence: {
        ...
        recentAdjudicationsOnException: [],
        adjudicationOutcomeCounts: {},
        similarResolvedCases: [],
        recurrenceReasonCodes: ["family_history_building"],
      },
      proofLineage: {
        runId: RUN_ID,
        evidenceArtifactIds: ["ev-1"],
        proofPackageIds: ["proof-1"],
        adjudicationMemoryIds: [],
        priorRunResultId: null,
      },
    });
  ...
  test("returns canonical exception detail with durable memory, evidence, and proof summaries", async () => {
    getReconciliationWorkbenchExceptionDetailMock.mockResolvedValue({
      ...
      runComparison: {
        ...
        certainty: "high",
        reasonCodes: [],
        summary: "Compared to prior run.",
        baseline: { priorResultId: "prior-res-1", priorResultStartedAt: "2025-12-31T00:00:00.000Z" },
        deltas: {
          matched: 1,
          unmatched: 0,
          conflicts: 0,
      ...
      exceptionIntelligence: {
        ...
        adjudicationOutcomeCounts: { resolved: 1 },
        similarResolvedCases: [],
        recurrenceReasonCodes: [],
      },
      proofLineage: {
        runId: RUN_ID,
        evidenceArtifactIds: ["ev-1"],
        proofPackageIds: ["proof-1"],
        adjudicationMemoryIds: ["mem-1"],
        priorRunResultId: "prior-res-1",
      },
      auditTrail: [
        {
