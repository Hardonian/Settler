...
describe('status API degraded-mode contract', () => {
  ...
  it('keeps /api/status/health returning 200 degraded envelope instead of hard 500', () => {
    const source = readFileSync(resolve(process.cwd(), 'src/app/api/status/health/route.ts'), 'utf-8');

    expect(source).toContain("status: 'Building Momentum'");
    expect(source).toContain("allCylindersFiring: false");
    expect(source).toContain("error: 'Unable to fetch health status'");
    expect(source).toContain('{ status: 200 }');
  });
});
