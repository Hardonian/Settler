...
export async function enforceUsageLimit(
  ...
  try {
    ...
    if (!result.allowed) {
      ...
      await recordUsageEvent(auth.billingAccountId, service, "request_blocked", quantity, {
        pathname,
      ...
      return {
        ...
        response: NextResponse.json(
          createErrorResponse("USAGE_LIMIT_EXCEEDED", result.reason || "Usage limit exceeded", 429),
          { status: 429 }
        ),
      };
    }
    ...
    recordUsageEvent(auth.billingAccountId, service, "request_allowed", quantity, {
      pathname,
