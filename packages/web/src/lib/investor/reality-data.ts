...
export async function getInvestorRealityData(): Promise<InvestorRealityData> {
  ...
  try {
    ...
    const pick = (category: string, name: string) =>
      rows.find((row) => row.category === category && row.name === name);
    ...
    const evidenceSignals = [
      revenueStatus,
      usageStatus,
      pick("failure", "hard_500_count")?.status ?? "assumed",
    ];
    ...
    return {
      ...
      evidence_index: Math.round(
        (evidenceSignals.filter((status) => status === "proven").length / evidenceSignals.length) *
          100
      ),
