...
export async function flushApiCallLogs(): Promise<void> {
  ...
  try {
    ...
    const values = logs.map(
      (log) =>
        Prisma.sql`(${log.tenantId ?? null}, ${log.userId ?? null}, ${log.method}, ${log.path}, ${log.statusCode}, ${log.responseTime}, ${log.userAgent ?? null}, ${log.ipAddress ?? null}, ${log.error ?? null}, ${log.createdAt}::timestamp)`
    );
...
async function syncWriteApiCallLog(log: ApiCallLogBuffer): Promise<void> {
  try {
    await prisma.$executeRaw`
      INSERT INTO api_call_logs (tenant_id, user_id, method, path, status_code, response_time, user_agent, ip_address, error, created_at)
      VALUES (${log.tenantId ?? null}, ${log.userId ?? null}, ${log.method}, ${log.path}, ${log.statusCode}, ${log.responseTime}, ${log.userAgent ?? null}, ${log.ipAddress ?? null}, ${log.error ?? null}, ${log.createdAt}::timestamp)
    `;
  } catch (error) {
