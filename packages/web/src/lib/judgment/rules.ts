...
function detectRepeatedDrift(
  ...
  const totalDelta =
    last7Days.reduce((sum: number, h: any) => sum + Math.abs(h.delta), 0) + Math.abs(delta);
...
function detectComplianceBreach(event: MeaningfulChange["event"]): {
  breached: boolean;
  reason?: string;
} {
...
export function generateExplanation(
  ...
  if (delta !== undefined) {
    ...
    if (severity === "critical") {
      whyItMatters =
        "This is a critical financial discrepancy that requires immediate attention. Large deltas can indicate systematic errors, fraud, or data quality issues.";
      suggestedNextStep = "Review source data immediately and escalate to finance team";
    } else if (severity === "high") {
      whyItMatters =
        "This significant delta suggests a potential issue with data synchronization or accounting accuracy.";
      suggestedNextStep = "Investigate source and target systems for data quality issues";
    } else {
      whyItMatters =
        "This delta may indicate minor synchronization issues or timing differences between systems.";
