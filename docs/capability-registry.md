...
| ------------------------------------------------ | -------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------- |
| Reconciliation execution                         | Deterministic compute + reconciliation runtime     | `/api/v1/runs`, `/api/runs/create`                                              | `pnpm demo:settler`, `pnpm simulate:settler`                         | `/console/runs`, `/console/reconciliations`                             | `docs/reference/surface-area-convergence.md`                                  | Core in all tiers                      |
| Run explorer                                     | Run indexing + run evidence links                  | `/api/v1/runs/:id`, `/api/admin/runs`                                           | `pnpm replay:run`                                                    | `/console/runs`, `/console/runs/[runId]`, `/admin/runs`                 | `docs/reference/surface-area-convergence.md`                                  | High (operator workflows)              |
| Truth explorer / proof explorer                  | Evidence hashing + lineage graph primitives        | `/api/v1/runs/:id/trust-explorer/*`, `/api/v1/runs/[id]/evidence`               | `pnpm replay:run`, `proof show`, `proof verify`                      | `/console/proof-explorer`                                               | `docs/architecture/proof-explorer.md`, `docs/enterprise-capability-matrix.md` | Critical (auditability/compliance)     |
...
| Policy simulation and governance                 | Policy simulation + policy validation hooks        | `/api/control-plane/policies`, `/api/control-plane/triggers`                    | `policy simulate`, `policy validate`, `pnpm simulate:settler`        | `/console/policies`, `/console/control-plane`                           | `docs/enterprise-capability-matrix.md`                                        | Critical (governance)                  |
...
| Operational monitoring                           | Kernel telemetry + health checks                   | `/api/metrics`, `/api/ops/system-health`, `/api/console/metrics`, `/api/status` | `pnpm doctor`, `pnpm suite-doctor:json`, `console health`            | `/console`, `/console/performance`, `/console/diagnostics`              | `docs/operational-safety-audit.md`, `docs/enterprise-console-surface-map.md`  | Critical (operations)                  |
...
| Tenant governance and isolation                  | Tenant-scoped authz checks                         | `/api/rbac/*`, `/api/console/tenants`, tenant-scoped `/api/*`                   | `pnpm tenant:create`, `pnpm verify:tenant`, `pnpm test:cross-tenant` | `/console/organizations`, `/console/admin/tenants`, `/console/settings` | `docs/ACCESS_CONTROLS.md`, `docs/OSS_VS_ENTERPRISE.md`                        | Critical (multi-tenant safety)         |
