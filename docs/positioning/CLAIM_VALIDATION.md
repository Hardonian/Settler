...
| 3   | "Every result is provable — proof artifacts are generated automatically"          | README                 | Yes               | Proof artifact generation is part of run execution. Artifacts include inputs, rules, outputs, and hashes.                                                                             | Kept — accurate                                                                                                           |
| 4   | "Replayable execution"                                                            | README, homepage, docs | Yes               | Replay functionality re-executes a run against the same inputs and config. Output is verified by `scripts/verify-proof.ts` and replay checks.                                         | Kept — accurate                                                                                                           |
| 5   | "Proof artifact generation"                                                       | README, homepage       | Yes               | `evidence.json` is generated per run containing input snapshot, rule config, execution trace, and output fingerprint.                                                                 | Kept — accurate                                                                                                           |
...
| 8   | "Tenant isolation controls include row-level security"                            | Docs, security         | Partially         | RLS definitions and application guards are verified in the repository; live target RLS evidence is pending.                                                                           | Kept with an explicit runtime boundary                                                                                    |
| 9   | "Cryptographic hashing of inputs and outputs"                                     | Docs                   | Yes               | Content hashing is implemented for proof artifacts using SHA-256 canonical fingerprints and verified provenance chain checks.                                                         | Kept — accurate, described as "tamper-evident" not "immutable"                                                            |
...
| -------------------------- | ---------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| "Control plane"            | "Reconciliation platform with verifiable execution proofs" | "Open-source reconciliation engine"          | "Control plane" is infrastructure jargon. "Engine" is clearer.                                        |
...
- Layered tenant-isolation controls with target-environment RLS verification
...
- Event durability validated via append-only event log + replay tooling (`runner/eventBackbone.ts`, `scripts/stress-reliability.ts`)
