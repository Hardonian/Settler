...
| 4 | "Replayable runs" | README, homepage, docs | Yes | Replay functionality re-executes a run against the same inputs and config. Output can be compared for bitwise equality. | Kept — accurate |
...
| 9 | "Cryptographic hashing of inputs and outputs" | Docs | Yes | Content hashing is implemented for evidence bundles. Hashes are SHA-256 based. | Kept — accurate, described as "tamper-evident" not "immutable" |
