...
| Variable | Status | Required | Description | Notes |
|----------|--------|----------|-------------|-------|
| `NEXT_PUBLIC_SUPABASE_URL` | ⚠️ | Yes | Public Supabase URL | **ISSUE:** Referenced as `secrets.NEXT_PUBLIC_SUPABASE_URL` in supabase-migrate.yml line 352, but GitHub secrets don't typically use NEXT_PUBLIC_ prefix. Should use `SUPABASE_URL` instead. |
