...
4. **Any code using process.env.SUPABASE\_\*** - Code references

### Migration Steps

1. Update Doppler secrets (new names without SUPABASE\_ prefix)
...
// NEW
const supabaseUrl =
  process.env.DATABASE_URL?.replace("/postgres", "") || process.env.NEXT_PUBLIC_SUPABASE_URL;
