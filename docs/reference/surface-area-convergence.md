...
| Deterministic replay | packages/cli/src/tools/replay-runner.ts<br/>packages/web/src/app/replay-lab/page.tsx | pnpm replay:run | /replay-lab<br/>/app/runs/[id] | /api/v1/runs/:id/replay | docs/replay/README.md | OSS + operator | Requires prior run evidence for deterministic compare. | mature |
...
| Operator telemetry and capability status | scripts/doctor.mjs<br/>packages/web/src/lib/status/runtime-connectivity-health.ts<br/>packages/web/src/app/api/status/route.ts | pnpm doctor<br/>pnpm suite-doctor:json | /app/capability-status<br/>/status<br/>/app/system-health | /api/status<br/>/api/status/health | docs/launch/canonical-go-live-path.md | OSS + operator | `/api/status/health` returns `settler.runtime_connectivity` (DB + Supabase + env probes only; not KPIs or uptime %). | mature |
